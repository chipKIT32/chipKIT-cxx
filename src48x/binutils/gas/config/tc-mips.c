...
#define MIPS_CPU_ASE_EVA	0x0400  /* CPU implements EVA */
...
macro (struct mips_cl_insn *ip, char *str)
{
  ...
  switch (mask)
      ...
      if (offset_expr.X_op == O_constant)
	{
	  ...
	  if (!off9 && !off12 && !IS_SEXT_16BIT_NUM (expr1.X_add_number))
	    {
	      expr1.X_add_number = ((expr1.X_add_number + 0x8000)
				    & ~(bfd_vma) 0xffff);
	      hipart = 1;
	    }
	  else if (!off9 && off12 && !IS_SEXT_12BIT_NUM (expr1.X_add_number))
	    {
	      expr1.X_add_number = ((expr1.X_add_number + 0x800)
				    & ~(bfd_vma) 0xfff);
	      hipart = 1;
	    }
	  else if (off9 && !off12 && !IS_SEXT_9BIT_NUM (expr1.X_add_number))
	    {
      ...
      else if (off0 || off9 || off12)
	{
	  /* A 0, 9 or 12-bit offset field is too narrow to be used
	     ...
	     we first load absolute address "A" into the register and
             ...
             we simply need to add 16-bit offset "o" to base register "b",
             and offset_reloc already contains the relocations associated
	     with "o".  */
	  if (ab)
	    {
...
#define USE_BITS(mask,shift)	(used_bits |= ((mask) << (shift)))
  while (*p)
	...
	break;
      ...
      case '#': USE_BITS (OP_MASK_CODE10,	OP_SH_CODE10);	break;
  ...
  for (;;)
      ...
      for (args = insn->args;; ++args)
	{
	  ...
	  switch (*args)
	    ...
	    case 'B':		/* 20-bit syscall/breakpoint code.  */
	      gas_assert (!mips_opts.micromips);
	      my_getExpression (&imm_expr, s);
	      check_absolute_expr (ip, &imm_expr);
	      if ((unsigned long) imm_expr.X_add_number > OP_MASK_CODE20)
		as_warn (_("Code for %s not in range 0..%lu (%lu)"),
			 ip->insn_mo->name, (unsigned long) OP_MASK_CODE20,
			 (unsigned long) imm_expr.X_add_number);
	    ...
	    case '#':	/* 10-bit syscall/wait/breakpoint/hypcall code.  */
	      {
		unsigned long mask = (mips_opts.micromips
		...
		if ((unsigned long) imm_expr.X_add_number > mask)
		  as_warn (_("Code for %s not in range 0..%lu (%lu)"),
			   ...
			   mask, (unsigned long) imm_expr.X_add_number);
		INSERT_OPERAND (mips_opts.micromips,
				CODE10, *ip, imm_expr.X_add_number);
		imm_expr.X_op = O_absent;
		s = expr_end;
	      }
	      continue;
		      ...
		      || imm_expr.X_add_number >= 0x10000))
		{
		  if (imm_expr.X_op == O_constant || imm_expr.X_op == O_big)
		    as_bad (_("lui expression (%lu) not in range 0..65535"),
			    (unsigned long) imm_expr.X_add_number);
		  else if (imm_expr.X_op != O_subtract)
		    insn_error = _("Absolute expression required");
		}
	      s = expr_end;
	      continue;
...
static const struct mips_cpu_info mips_cpu_info_table[] =
{
  ...
  /* Netlogic/Broadcom XLP */
  { "xlp",	      0,      ISA_MIPS64R2,   CPU_XLP },

  /* Microchip Technology PIC32 MCUs */
  { "pic32mx",         MIPS_CPU_ASE_DSP | MIPS_CPU_ASE_DSPR2 | MIPS_CPU_ASE_MCU,
...
mips_cfi_endproc (struct fde_entry *fde ATTRIBUTE_UNUSED)
{
  ...
  bfd_boolean seen_epilogue_begin = FALSE;

  for (reg = 0; reg < MIPS_NUM_UNWIND_REGS; reg++)
    reg_offset[reg] = 0;

  /* Scan FDE instructions to build up stack frame layout.  */
  for (insn = fde->data; insn && !seen_epilogue_begin; insn = insn->next)
    {
      switch (insn->insn)
	{
	...
	case CFI_epilogue_begin:
	  /* For compact unwinding tables, CFI directives
             in the epilogue are not processed.  */
	  seen_epilogue_begin = TRUE;
	  break;
