...
/*  Emit the 'duplicate member' error only when the duplicate member is at a different 
    location within the anonymous struct from the original member.
 */
...
match_duplicate_member(tree fieldlist, tree duplicate_member, int level)
{
  ...
  for (x = fieldlist; x; x = DECL_CHAIN (x))
  {
    if ((DECL_NAME(x) != NULL) && (DECL_NAME(duplicate_member) != NULL) &&
      ...
      { 
          match_duplicate_member(TYPE_FIELDS (TREE_TYPE (x)), duplicate_member, ++level);
          --level; 
      }
  }
}
...
detect_field_duplicates_hash (tree fieldlist,
			      hash_table <pointer_hash <tree_node> > htab, tree original_fieldlist, enum tree_code code)
#else
detect_field_duplicates_hash (tree fieldlist,
...
#endif
{
  ...
  for (x = fieldlist; x ; x = DECL_CHAIN (x))
	...
	*slot = y;
      ...
      {
#if defined(TARGET_MCHP_PIC32MX)
       detect_field_duplicates_hash (TYPE_FIELDS (TREE_TYPE (x)), htab, original_fieldlist, code);
#else
