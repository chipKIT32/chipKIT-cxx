...
extern char *pic32_resource_file_f;  /* stoopid options.c always makes this const */
#endif
...
/* Begin _BUILD_MCHP_ _BUILD_C32_ _BUILD_XC32_ */
#ifndef STARTFILECXX_SPEC
#define STARTFILECXX_SPEC  STARTFILE_SPEC
#endif
/* End _BUILD_MCHP_ _BUILD_C32_ _BUILD_XC32_ */
...
/* _BUILD_MCHP_ _BUILD_C32_ _BUILD_XC32_ */
...
{
  char *string,*prev_string;
  prev_string = (char*)argbuf.pop();

   
  /* not sure where we got the memory for the string, so we alloc
     new space to be sure we have room to append. */
  string = (char*)xmalloc (strlen (arg) + strlen (prev_string) + 1 + 1);
  strcpy (string, prev_string);
  strcat (string, arg);
  
  argbuf.safe_push (string);
...
#ifdef _BUILD_MCHP_
   gcc_exec_prefix = getenv (GCC_EXEC_PREFIX_ENV);
#else
...
do_spec_1 (const char *spec, int inswitch, const char *soft_matched_part)
{
  ...
  while ((c = *p++))
	...
	switch (c = *p++)
      ...
      /* _BUILD_MCHP_ _BUILD_C32_ _BUILD_XC32 */
	  case 'E':
	    if (strstr (progname,"-gcc") != NULL)
	      value = do_spec_1 (endfile_spec, 0, NULL);
	    else if (strstr (progname,"-g++") != NULL)
