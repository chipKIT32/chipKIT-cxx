...
/* Return TRUE if ST_TYPE identifies a Thumb function, and FALSE for an
   ARM function.  If we aren't sure (i.e. this is not a function symbol)
   then return GUESS.  Used for branch relocations.  */
static bfd_boolean
is_thumb_func (int st_type, bfd_boolean guess)
{
  ...
  if (r_type == R_ARM_THM_CALL || r_type == R_ARM_THM_JUMP24
      || r_type == R_ARM_THM_TLS_CALL)
    {
      ...
      */
	  ...
	  || (!is_thumb_func(st_type, TRUE)
	      && (((r_type == R_ARM_THM_CALL) && !globals->use_blx)
		  || (r_type == R_ARM_THM_JUMP24))
	      && !use_plt))
	{
	  if (is_thumb_func(st_type, TRUE) && r_type != R_ARM_THM_TLS_CALL)
	    {
	      /* Thumb to thumb.  */
	      if (!thumb_only)
		{
    ...
    {
      if (is_thumb_func (st_type, FALSE))
	{
	  /* Arm to thumb.  */
...
#define MAXRELOCS 2
  ...
  /* ??? For non-function symbols this should depend whether the caller
     is ARM or Thumb.  However we only have one stub per symbol/section.  */
  if (is_thumb_func(stub_entry->st_type, FALSE))
    sym_value |= 1;
      ...
      for (span = 0; span < sec_data->mapcount; span++)
        {
		  ...
		  if (found)
			...
			{
			  if (use_plt || !is_thumb_func (found->st_type, TRUE))
	  ...
	  switch (r_type)
	    ...
	    case R_ARM_PC24:
	      /* This one is a call from arm code.  We need to look up
	         ...
	         insert glue.  */
	      if (is_thumb_func (ELF_ST_TYPE (h->type), FALSE))
			       ...
			       char **                      error_message)
{
  ...
  switch (r_type)
	...
	case R_ARM_PLT32:
	  ...
	  bfd_boolean jump_to_next = FALSE;
	  ...
	  if (r_type == R_ARM_XPC25)
	  ...
	  else if (r_type == R_ARM_PC24)
	    {
	      ...
	      if (is_thumb_func (sym_flags, FALSE))
		{
	  ...
	  if (   r_type == R_ARM_CALL
	      ...
	      || r_type == R_ARM_PLT32)
	    {
	      ...
	      /* A branch to an undefined weak symbol is turned into a
	         jump to the next instruction unless a PLT entry will be
	         created.  Do the same for local undefined symbols (but not for
		 STN_UNDEF).  */
	  ...
	  if (jump_to_next)
	      ...
	      if (r_type == R_ARM_CALL)
		{
		  ...
		  /* Only if we are not handling a BL to a stub. In this
		     ...
		     If this is not a function symbol then leave it alone.  */
		  if (is_thumb_func (sym_flags, FALSE) && !stub_entry)
		    value |= (1 << 28);
		  else if (stub_entry || !is_thumb_func (sym_flags, TRUE))
		    {
		      value &= ~(bfd_vma)(1 << 28);
		      value |= (1 << 24);
		    }
	...
	if (r_type == R_ARM_THM_XPC22)
	...
	else
	  {
	    ...
	    if (!is_thumb_func (sym_flags, TRUE) && sym_flags != STT_SECTION
		&& (h == NULL || splt == NULL
		    || h->plt.offset == (bfd_vma) -1))
	      {
	    ...
	    else if (is_thumb_func (sym_flags, FALSE) && globals->use_blx
		     && r_type == R_ARM_THM_CALL)
	      {
		...
		if (need_relocs)
		  {
		    ...
		    loc = globals->srelgot->contents;
		    loc += (globals->srelgot->reloc_count++
			    * RELOC_SIZE (globals));
	    ...
	    || ELF32_R_TYPE(rel->r_info) == R_ARM_THM_TLS_CALL)
	  {
	    ...
	    enum elf32_arm_stub_type stub_type
	      = arm_type_of_stub (info, input_section, rel, &sym_flags,
				  (struct elf32_arm_link_hash_entry *)h,
				  globals->tls_trampoline, globals->splt,
				  input_bfd, sym_name);
		...
		struct elf32_arm_stub_hash_entry *stub_entry
		  ...
		  (input_section, globals->splt, 0, rel, globals, stub_type);
		offset = (stub_entry->stub_offset
			  + stub_entry->stub_sec->output_offset
			  + stub_entry->stub_sec->output_section->vma);
	      }
	    else
	      offset = (globals->splt->output_section->vma
			+ globals->splt->output_offset
		...
		value = inst | (globals->use_blx ? 0xfa000000 : 0xeb000000);
	      ...
	      {
		/* Thumb blx encodes the offset in a complicated fashion.  */
	...
	/* These relocations needs special care, as besides the fact
	  ...
	  value = ((globals->sgot->output_section->vma
		    + globals->sgot->output_offset + off)
		   - (input_section->output_section->vma
		      + input_section->output_offset + rel->r_offset));
	...
	case Tag_ABI_align_needed:
	  if ((in_attr[i].i > 0 || out_attr[i].i > 0)
		  ...
		  || out_attr[Tag_ABI_align_preserved].i == 0))
	    {
		 ...
		 properly.
	      _bfd_error_handler
		(_("error: %B: required data alignment conflicts with %B"),
		 obfd, ibfd);
	      result = FALSE; */
	    }
	  /* Fall through.  */
...
elf32_arm_finish_dynamic_sections (bfd * output_bfd, struct bfd_link_info * info)
{
  ...
  if (elf_hash_table (info)->dynamic_sections_created)
      ...
      if (htab->dt_tlsdesc_plt)
	{
	  ...
	  bfd_vma gotplt_address = (htab->sgot->output_section->vma
