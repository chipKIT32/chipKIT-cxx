  ...
  if (elf_m68k_hash_entry (h)->glist != NULL)
      ...
      while (got_entry != NULL)
	{
	  ...
	  if (info->shared
	      && SYMBOL_REFERENCES_LOCAL (info, h))
	    {
	      enum elf_m68k_reloc_type reloc_got_type;
	      void *relocation_addr;
	      bfd_vma relocation;

	      reloc_got_type = elf_m68k_reloc_got_type (r_type);

	      relocation_addr = sgot->contents + got_entry_offset;
	      /* GDxx GOT entries have RELOCATION located in the 2nd slot of
		 the 2-slot entry.  */
	      relocation_addr += reloc_got_type == R_68K_TLS_GD32 ? 4 : 0;

	      relocation = bfd_get_signed_32 (output_bfd, relocation_addr);

	      /* Undo TP bias.  */
	      switch (reloc_got_type)
		{
		case R_68K_GOT32O:
		case R_68K_TLS_LDM32:
		  break;
