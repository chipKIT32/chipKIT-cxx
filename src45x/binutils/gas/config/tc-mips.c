...
/* True if -mmicromips was passed or implied by arguments passed on the
   command line (e.g., by -march).  */
...
static struct mips_cl_insn nop_insn, mips16_nop_insn, micromips_nop16_insn,
...
static const int micromips_imm_b_map[16] =
{
  1, 4, 8, 12, 16, 20, 24, -1
};
...
#define RELAX_DELAY_SLOT_SIZE_ERROR_FIRST 0x100000
#define RELAX_DELAY_SLOT_SIZE_ERROR_SECOND 0x200000
...
#define MICROMIPS_INSERT_OPERAND(FIELD, INSN, VALUE) \
  INSERT_BITS ((INSN).insn_opcode, VALUE, \
...
#define MIPS16_EXTRACT_OPERAND(FIELD, INSN) \
  EXTRACT_BITS ((INSN).insn_opcode, \
		...
		MIPS16OP_SH_##FIELD)
#define MICROMIPS_EXTRACT_OPERAND(FIELD, INSN) \
  EXTRACT_BITS ((INSN).insn_opcode, \
		MICROMIPSOP_MASK_##FIELD, \
		MICROMIPSOP_SH_##FIELD)

#define A_BFD_RELOC_HI16_S \
  (mips_opts.micromips ? BFD_RELOC_MICROMIPS_HI16_S : BFD_RELOC_HI16_S)
#define A_BFD_RELOC_HI16 \
  (mips_opts.micromips ? BFD_RELOC_MICROMIPS_HI16 : BFD_RELOC_HI16)
#define A_BFD_RELOC_LO16 \
  (mips_opts.micromips ? BFD_RELOC_MICROMIPS_LO16 : BFD_RELOC_LO16)
#define A_BFD_RELOC_GPREL16 \
  (mips_opts.micromips ? BFD_RELOC_MICROMIPS_GPREL16 : BFD_RELOC_GPREL16)
#define A_BFD_RELOC_MIPS_GOT16 \
  (mips_opts.micromips ? BFD_RELOC_MICROMIPS_GOT16 : BFD_RELOC_MIPS_GOT16)
#define A_BFD_RELOC_MIPS_GOT_HI16 \
  (mips_opts.micromips ? BFD_RELOC_MICROMIPS_GOT_HI16 : BFD_RELOC_MIPS_GOT_HI16)
#define A_BFD_RELOC_MIPS_GOT_LO16 \
  (mips_opts.micromips ? BFD_RELOC_MICROMIPS_GOT_LO16 : BFD_RELOC_MIPS_GOT_LO16)
#define A_BFD_RELOC_MIPS_HIGHEST \
  (mips_opts.micromips ? BFD_RELOC_MICROMIPS_HIGHEST : BFD_RELOC_MIPS_HIGHEST)
#define A_BFD_RELOC_MIPS_HIGHER \
  (mips_opts.micromips ? BFD_RELOC_MICROMIPS_HIGHER : BFD_RELOC_MIPS_HIGHER)
#define A_BFD_RELOC_MIPS_GOT_DISP \
  (mips_opts.micromips ? BFD_RELOC_MICROMIPS_GOT_DISP : BFD_RELOC_MIPS_GOT_DISP)
#define A_BFD_RELOC_MIPS_GOT_PAGE \
  (mips_opts.micromips ? BFD_RELOC_MICROMIPS_GOT_PAGE : BFD_RELOC_MIPS_GOT_PAGE)
#define A_BFD_RELOC_MIPS_GOT_OFST \
  (mips_opts.micromips ? BFD_RELOC_MICROMIPS_GOT_OFST : BFD_RELOC_MIPS_GOT_OFST)
#define A_BFD_RELOC_MIPS_SUB \
  (mips_opts.micromips ? BFD_RELOC_MICROMIPS_SUB : BFD_RELOC_MIPS_SUB)
#define A_BFD_RELOC_MIPS_JALR \
  (mips_opts.micromips ? BFD_RELOC_MICROMIPS_JALR : BFD_RELOC_MIPS_JALR)

...
static struct {
  ...
  /* For relaxable macros, num_insns[0] is the number of instructions for the
     first alternative and num_insns[1] is the number of instructions for the
     second alternative.
     For non-relaxable macros, both elements give the number of instructions
     for the macro.  */
  unsigned int num_insns[2];
...
static void micromips_macro_build
  (expressionS *, const char *, const char *, va_list *);
...
static void micromips_macro (struct mips_cl_insn * ip);
#ifdef LOSING_COMPILER
static void macro2 (struct mips_cl_insn * ip);
#endif
static void mips_ip (char *str, struct mips_cl_insn * ip);
static void micromips_ip (char *str, struct mips_cl_insn * ip);
...
/* These are set by micromips_ip if an explicit size is used.  */
static bfd_boolean micromips_16, micromips_32;
...
static bfd_boolean
is_micromips_16bit_p (const struct mips_opcode *mo)
{
  /* If the upper 16-bit of the mask isn't 0, it isn't a 16-bit
     instruction.  */
  if ((mo->mask >> 16) != 0)
    return FALSE;

  /* Make sure major opcode belongs to 16-bit instructions.  */
  if (((mo->match >> 10) & 7) != 1
      && ((mo->match >> 10) & 7) != 2
      && ((mo->match >> 10) & 7) != 3)
    abort ();
...
static bfd_boolean
is_micromips_32bit_p (const struct mips_opcode *mo)
{
  /* If the upper 16-bit of the mask is 0, it isn't a 32-bit instruction.  */
  if ((mo->mask >> 16) == 0)
    return FALSE;

  /* Make sure major opcode doesn't belong to 16-bit instructions.  */
  if (((mo->match >> 26) & 7) == 1
      || ((mo->match >> 26) & 7) == 2
      || ((mo->match >> 26) & 7) == 3)
    abort ();

  /* Check if it is not 48-bit instructions.  */
  return ((mo->match >> 26) & 0x3f) != 0x1f;
}
...
static void
mips_record_mips16_micromips_mode (void)
{
  segment_info_type *si;
...
install_insn (const struct mips_cl_insn *insn)
{
  ...
  mips_record_mips16_micromips_mode ();
}
...
is_opcode_valid (const struct mips_opcode *mo)
{
  ...
  if (mips_opts.micromips)
    {
      if (micromips_16 == TRUE)
	{
	  if (mo->pinfo == INSN_MACRO || !is_micromips_16bit_p (mo))
	    return FALSE;
	}
      else if (micromips_32 == TRUE)
	{
	  if (mo->pinfo == INSN_MACRO || !is_micromips_32bit_p (mo))
	    return FALSE;
	}
      else
	{
	  /* When we are not expanding macros, we will check if instructions
	     can be in the delay slot with required size.  */
	  if ((history[0].insn_mo->pinfo2 & INSN2_BRANCH_DELAY_32BIT)
	      && mo->pinfo != INSN_MACRO
	      && !is_micromips_32bit_p (mo))
	    return FALSE;

	  if ((history[0].insn_mo->pinfo2 & INSN2_BRANCH_DELAY_16BIT)
	      && mo->pinfo != INSN_MACRO
	      && !is_micromips_16bit_p (mo))
	    return FALSE;
	}
    }
...
static void
mips16_micromips_mark_labels (void)
{
  segment_info_type *si = seg_info (now_seg);
  struct insn_label_list *l;
  ...
  for (l = si->label_list; l != NULL; l = l->next)
   {
	...
	/* Don't adjust the address if the label is global or weak, or
	  ...
	  && (mips_opts.mips16 || mips_opts.micromips))
	     ...
	     bfd_reloc_code_real_type *reloc_type)
{
  ...
  /* The value passed to dwarf2_emit_insn is the distance between
     ...
     should be recorded in the debug tables.  For MIPS16/microMIPS debug info
     we want to use ISA-encoded addresses, so we pass -1 for an
     address higher by one than the current.  */
  dwarf2_emit_insn ((mips_opts.mips16 || mips_opts.micromips) ? -1 : 0);
#endif
  ...
  if (address_expr
      ...
      && !mips_opts.micromips)
    {
      ...
      *reloc_type = BFD_RELOC_UNUSED;
	   ...
	   && *reloc_type == BFD_RELOC_MICROMIPS_16_PCREL_S1
	   && (pinfo & INSN_UNCOND_BRANCH_DELAY
	   ...
	   && !micromips_32)
    {
      add_relaxed_insn (ip, (relaxed_micromips_32bit_branch_length
			     (NULL, NULL,
			      (pinfo & INSN_UNCOND_BRANCH_DELAY) ? -1
			      : 0)), 4,
			RELAX_MICROMIPS_ENCODE
			(0, 0,
			 pinfo & INSN_UNCOND_BRANCH_DELAY,
			 pinfo & INSN_WRITE_GPR_31,
			 0),
  ...
  else if (mips_opts.micromips && *reloc_type > BFD_RELOC_UNUSED)
    {
      ...
      if (mips_relax.sequence != 2)
	{
	  ...
	  mips_macro_warning.num_insns[0]++;
	}
  ...
  if (address_expr != NULL && *reloc_type <= BFD_RELOC_UNUSED)
      ...
      if (address_expr->X_op == O_constant)
	{
	  ...
	  switch (*reloc_type)
		...
		as_bad (_("jump to misaligned address (0x%lx)"),
			(unsigned long) address_expr->X_add_number);
	      ip->insn_opcode |= (address_expr->X_add_number >> 1) & 0x3ffffff;
	      break;
		...
		as_bad (_("branch to misaligned address (0x%lx)"),
	      ...
	      if (mips_relax_branch)
		goto need_reloc;
	      if ((address_expr->X_add_number + 0x10000) & ~0x1ffff)
		as_bad (_("branch address range overflow (0x%lx)"),
			(unsigned long) address_expr->X_add_number);
	      ip->insn_opcode |= (address_expr->X_add_number >> 1) & 0xffff;
	      break;
	...
	need_reloc:
	{
	  ...
	  ip->fixp[0] = fix_new_exp (ip->frag, ip->where,
				     ...
				     || reloc_type[0] == BFD_RELOC_MICROMIPS_16_PCREL_S1,
				     reloc_type[0]);
	  ...
	  if (HAVE_64BIT_GPRS
		  ...
		  || reloc_type[0] == BFD_RELOC_MICROMIPS_16
		  || reloc_type[0] == BFD_RELOC_MICROMIPS_JMP
  ...
  if (! mips_opts.mips16 && ! mips_opts.micromips)
    ...
    {
      if (pinfo & INSN_WRITE_GPR_D)
	mips_gprmask |= 1 << MICROMIPS_EXTRACT_OPERAND (RD, *ip);
      if ((pinfo & (INSN_WRITE_GPR_T | INSN_READ_GPR_T)) != 0)
	mips_gprmask |= 1 << MICROMIPS_EXTRACT_OPERAND (RT, *ip);
      if ((pinfo & INSN_READ_GPR_S)
	  || (pinfo2 & INSN2_WRITE_GPR_S))
	mips_gprmask |= 1 << MICROMIPS_EXTRACT_OPERAND (RS, *ip);
      if (((pinfo & INSN_WRITE_GPR_31) | (pinfo2 & INSN2_READ_GPR_31)) != 0)
	mips_gprmask |= 1 << RA;
      if (pinfo & INSN_WRITE_FPR_D
	  || (pinfo2 & INSN2_READ_FPR_D))
	mips_cprmask[1] |= 1 << MICROMIPS_EXTRACT_OPERAND (FD, *ip);
      if ((pinfo & (INSN_WRITE_FPR_S | INSN_READ_FPR_S)) != 0)
	mips_cprmask[1] |= 1 << MICROMIPS_EXTRACT_OPERAND (FS, *ip);
      if ((pinfo & (INSN_WRITE_FPR_T | INSN_READ_FPR_T)) != 0)
	mips_cprmask[1] |= 1 << MICROMIPS_EXTRACT_OPERAND (FT, *ip);
      if ((pinfo & INSN_READ_FPR_R) != 0)
	mips_cprmask[1] |= 1 << MICROMIPS_EXTRACT_OPERAND (FR, *ip);
      if (pinfo & INSN_COP)
	{
	  /* We don't keep enough information to sort these cases out.
	     The itbl support does keep this information however, although
	...
	mips_gprmask |= 1 << micromips_to_32_reg_b_map
			       [MICROMIPS_EXTRACT_OPERAND (MB, *ip)];
      if (pinfo2 & INSN2_MOD_GPR_MC)
	mips_gprmask |= 1 << micromips_to_32_reg_c_map
			       [MICROMIPS_EXTRACT_OPERAND (MC, *ip)];
      if (pinfo2 & INSN2_MOD_GPR_MD)
	mips_gprmask |= 1 << micromips_to_32_reg_d_map
			       [MICROMIPS_EXTRACT_OPERAND (MD, *ip)];
      if (pinfo2 & INSN2_MOD_GPR_ME)
	mips_gprmask |= 1 << micromips_to_32_reg_e_map
			       [MICROMIPS_EXTRACT_OPERAND (ME, *ip)];
      if (pinfo2 & INSN2_MOD_GPR_MF)
	mips_gprmask |= 1 << micromips_to_32_reg_f_map
			       [MICROMIPS_EXTRACT_OPERAND (MF, *ip)];
      if (pinfo2 & INSN2_MOD_GPR_MG)
	mips_gprmask |= 1 << micromips_to_32_reg_g_map
			       [MICROMIPS_EXTRACT_OPERAND (MG, *ip)];
      if (pinfo2 & INSN2_MOD_GPR_MHI)
	{
	  mips_gprmask |= 1 << micromips_to_32_reg_h_map
				 [MICROMIPS_EXTRACT_OPERAND (MH, *ip)];
	  mips_gprmask |= 1 << micromips_to_32_reg_i_map
				 [MICROMIPS_EXTRACT_OPERAND (MI, *ip)];
	}
      if (pinfo2 & INSN2_MOD_GPR_MJ)
	mips_gprmask |= 1 << MICROMIPS_EXTRACT_OPERAND (MJ, *ip);
      if (pinfo2 & INSN2_MOD_GPR_MM)
	mips_gprmask |= 1 << micromips_to_32_reg_m_map
			       [MICROMIPS_EXTRACT_OPERAND (MM, *ip)];
      if (pinfo2 & INSN2_MOD_GPR_MN)
	mips_gprmask |= 1 << micromips_to_32_reg_n_map
			       [MICROMIPS_EXTRACT_OPERAND (MN, *ip)];
      if (pinfo2 & INSN2_MOD_GPR_MP)
	mips_gprmask |= 1 << MICROMIPS_EXTRACT_OPERAND (MP, *ip);
      if (pinfo2 & INSN2_MOD_GPR_MQ)
	mips_gprmask |= 1 << micromips_to_32_reg_q_map
			       [MICROMIPS_EXTRACT_OPERAND (MQ, *ip)];
  ...
  if (emit_branch_likely_macro == TRUE)
    {
      emit_branch_likely_macro = FALSE;

      /* We need to generate a label.  */
      micromips_add_number_label ();
    }
...
macro_start (void)
{
  ...
  memset (&mips_macro_warning.num_insns, 0,
	  sizeof (mips_macro_warning.num_insns));
  mips_macro_warning.delay_slot_p = (mips_opts.noreorder
				     && (history[0].insn_mo->pinfo
					 & (INSN_UNCOND_BRANCH_DELAY
					    | INSN_COND_BRANCH_DELAY
					    | INSN_COND_BRANCH_LIKELY)) != 0);
  mips_macro_warning.delay_slot_16bit_p = (mips_opts.noreorder
    && (history[0].insn_mo->pinfo2 & INSN2_BRANCH_DELAY_16BIT) != 0);
  mips_macro_warning.delay_slot_32bit_p = (mips_opts.noreorder
    && (history[0].insn_mo->pinfo2 & INSN2_BRANCH_DELAY_32BIT) != 0);
}

/* Given that a macro is longer than one instruction or of the wrong
   size, return the appropriate warning for it.  Return null if no
   warning is needed.  SUBTYPE is a bitmask of RELAX_DELAY_SLOT,
   RELAX_DELAY_SLOT_SIZE_ERROR_FIRST, RELAX_DELAY_SLOT_SIZE_ERROR_SECOND,
   and RELAX_NOMACRO.  */
...
macro_end (void)
{
  ...
  if (mips_macro_warning.num_insns[0] > 1
      || mips_macro_warning.num_insns[1] > 1)
    {
      ...
      /* Set up the relaxation warning flags.  */
      subtype = 0;
      if (mips_macro_warning.num_insns[1] > mips_macro_warning.num_insns[0])
	...
	{
	  /* Either the macro has a single implementation or both
	     implementations are longer than 1 instruction.  Emit the
	     warning now.  */
...
macro_build (expressionS *ep, const char *name, const char *fmt, ...)
{
  ...
  for (;;)
	...
	case '\\':
	  INSERT_OPERAND (3BITPOS, insn, va_arg (args, int));
	  continue;

	case '~':
	  INSERT_OPERAND (OFFSET12, insn, va_arg (args, unsigned long));
	  continue;
...
macro_build_jalr (expressionS *ep)
{
  ...
  if (MIPS_JALR_HINT_P (ep))
    fix_new_exp (frag_now, f - frag_now->fr_literal,
		 4, ep, FALSE, A_BFD_RELOC_MIPS_JALR);
}
...
macro_build_lui (expressionS *ep, int regnum)
{
  ...
  bfd_reloc_code_real_type r[3]
    = {BFD_RELOC_UNUSED, BFD_RELOC_UNUSED, BFD_RELOC_UNUSED};
  const char *name = "lui";
  const char *fmt = mips_opts.micromips ? "s,u" : "t,u";
	 ...
	 __gnu_local_gp is used if mips_no_shared.  */
      gas_assert (mips_pic == NO_PIC
		  ...
		  && strcmp (S_GET_NAME (ep->X_add_symbol),
                             "__gnu_local_gp") == 0));
      *r = A_BFD_RELOC_HI16_S;
    }

  if (mips_opts.micromips)
    mo = hash_find (micromips_op_hash, name);
  else
  ...
  if (IS_SEXT_16BIT_NUM(ep->X_add_number))
    {
      ...
      macro_build (ep, op, "t,o(b)", treg, A_BFD_RELOC_LO16, breg);
    }
  else
    {
      ...
      macro_build (ep, op, "t,o(b)", treg, A_BFD_RELOC_LO16, AT);
...
load_register (int reg, expressionS *ep, int dbl)
{
  ...
  if (ep->X_op != O_big)
      ...
      if (IS_SEXT_16BIT_NUM (ep->X_add_number))
	{
	  ...
	  macro_build (ep, "addiu", "t,r,j", reg, 0, A_BFD_RELOC_LO16);
	  return;
	}
      else if (ep->X_add_number >= 0 && ep->X_add_number < 0x10000)
	{
	  /* We can handle 16 bit unsigned values with an ori to
             $zero.  */
	  macro_build (ep, "ori", "t,r,i", reg, 0, A_BFD_RELOC_LO16);
	  return;
	}
      else if ((IS_SEXT_32BIT_NUM (ep->X_add_number)))
	{
	  /* 32 bit values require an lui.  */
	  macro_build (ep, "lui", "t,u", reg, A_BFD_RELOC_HI16);
	  if ((ep->X_add_number & 0xffff) != 0)
	    macro_build (ep, "ori", "t,r,i", reg, reg, A_BFD_RELOC_LO16);
	  return;
	}
    }
  ...
  if (hi32.X_add_number == 0)
      ...
      if (hi32.X_add_number == (offsetT) 0xffffffff)
	{
	  if ((lo32.X_add_number & 0xffff8000) == 0xffff8000)
	    {
	      macro_build (&lo32, "addiu", "t,r,j", reg, 0, A_BFD_RELOC_LO16);
	      return;
	    }
	  if (lo32.X_add_number & 0x80000000)
	    {
	      macro_build (&lo32, "lui", "t,u", reg, A_BFD_RELOC_HI16);
	      if (lo32.X_add_number & 0xffff)
		macro_build (&lo32, "ori", "t,r,i", reg, reg, A_BFD_RELOC_LO16);
	      return;
	    }
	}
	  ...
	  if (shift != 0)
	    {
	      ...
	      macro_build (&tmp, "addiu", "t,r,j", reg, 0, A_BFD_RELOC_LO16);
	      if (bit != 0)
		{
		  bit += shift;
  ...
  if ((lo32.X_add_number & 0xffff0000) == 0)
      ...
      if (freg != 0)
	{
      ...
      macro_build (&mid16, "ori", "t,r,i", reg, freg, A_BFD_RELOC_LO16);
      macro_build (NULL, "dsll", "d,w,<", reg, reg, 16);
      freg = reg;
    }
  if ((lo32.X_add_number & 0xffff) != 0)
    macro_build (&lo32, "ori", "t,r,i", reg, freg, A_BFD_RELOC_LO16);
}
...
load_address (int reg, expressionS *ep, int *used_at)
{
  ...
  if (mips_pic == NO_PIC)
	  ...
	  if ((valueT) ep->X_add_number <= MAX_GPREL_OFFSET
	      && !nopic_need_relax (ep->X_add_symbol, 1))
	    {
	      ...
	      macro_build (ep, ADDRESS_ADDI_INSN, "t,r,j", reg,
			   mips_gp_register, A_BFD_RELOC_GPREL16);
	      relax_switch ();
	    }

	  if (*used_at == 0 && mips_opts.at)
	    {
	      macro_build (ep, "lui", "t,u", reg, A_BFD_RELOC_MIPS_HIGHEST);
	      macro_build (ep, "lui", "t,u", AT, A_BFD_RELOC_HI16_S);
	      macro_build (ep, "daddiu", "t,r,j", reg, reg,
			   A_BFD_RELOC_MIPS_HIGHER);
	      macro_build (ep, "daddiu", "t,r,j", AT, AT, A_BFD_RELOC_LO16);
	  ...
	  else
	    {
	      ...
	      macro_build (ep, "daddiu", "t,r,j", reg, reg,
			   A_BFD_RELOC_MIPS_HIGHER);
	      macro_build (NULL, "dsll", "d,w,<", reg, reg, 16);
	      macro_build (ep, "daddiu", "t,r,j", reg, reg, A_BFD_RELOC_HI16_S);
	      macro_build (NULL, "dsll", "d,w,<", reg, reg, 16);
	      macro_build (ep, "daddiu", "t,r,j", reg, reg, A_BFD_RELOC_LO16);
	    }
	  ...
	  if (ep->X_add_number)
	    {
	      ...
	      macro_build (ep, ADDRESS_LOAD_INSN, "t,o(b)", reg,
			   A_BFD_RELOC_MIPS_GOT_DISP, mips_gp_register);
      ...
      else
	{
	  ...
	  ep->X_add_number = 0;
	  macro_build (ep, ADDRESS_LOAD_INSN, "t,o(b)", reg,
		       A_BFD_RELOC_MIPS_GOT16, mips_gp_register);
	  load_delay_nop ();
	  relax_start (ep->X_add_symbol);
	  relax_switch ();
      ...
      if (HAVE_NEWABI)
	{
	  ...
	  ep->X_add_number = 0;
	  relax_start (ep->X_add_symbol);
	  macro_build (ep, "lui", "t,u", reg, A_BFD_RELOC_MIPS_GOT_HI16);
	  macro_build (NULL, ADDRESS_ADD_INSN, "d,v,t",
		       reg, reg, mips_gp_register);
	  macro_build (ep, ADDRESS_LOAD_INSN, "t,o(b)",
		       reg, A_BFD_RELOC_MIPS_GOT_LO16, reg);
	  if (ex.X_add_number < -0x8000 || ex.X_add_number >= 0x8000)
	    as_bad (_("PIC code offset overflow (max 16 signed bits)"));
	  ...
	  relax_switch ();
	  macro_build (ep, ADDRESS_LOAD_INSN, "t,o(b)", reg,
		       A_BFD_RELOC_MIPS_GOT_PAGE, mips_gp_register);
	  macro_build (ep, ADDRESS_ADDI_INSN, "t,r,j", reg, reg,
		       A_BFD_RELOC_MIPS_GOT_OFST);
	  relax_end ();
	}
      else
	{
...
add_got_offset (int dest, expressionS *local)
{
  ...
  macro_build (&global, ADDRESS_ADDI_INSN, "t,r,j",
	       dest, dest, A_BFD_RELOC_LO16);
  relax_switch ();
  macro_build (local, ADDRESS_ADDI_INSN, "t,r,j", dest, dest, A_BFD_RELOC_LO16);
  relax_end ();
}
...
add_got_offset_hilo (int dest, expressionS *local, int tmp)
{
  ...
  macro_build (local, ADDRESS_ADDI_INSN, "t,r,j", tmp, tmp, A_BFD_RELOC_LO16);
  relax_end ();
...
macro (struct mips_cl_insn *ip)
{
  ...
  switch (mask)
    ...
    all_mem_op:
      ...
      if (offset_expr.X_op == O_constant)
	{
	  load_register (AT, &offset_expr, HAVE_64BIT_ADDRESSES);
	  if (breg != 0)
	    macro_build (NULL, ADDRESS_ADD_INSN, "d,v,t", AT, AT, breg);
	}
	...
	abort ();
      ...
      macro_build (NULL, s, "\\,~(b)", treg, (unsigned long) 0, AT);
      break;
...
validate_micromips_insn (const struct mips_opcode *opc)
{
  ...
  if ((used_bits & opc->match) != opc->match)
    {
      as_bad (_("internal: bad micromips opcode (mask error): %s %s"),
	      opc->name, opc->args);
      return 0;
    }
  return 1;
      ...
      for (args = insn->args;; ++args)
	{
	  ...
	  switch (*args)
	    ...
	    case '\\':		/* 3-bit bit position */
	      my_getExpression (&imm_expr, s);
	      check_absolute_expr (ip, &imm_expr);
	      if ((unsigned long) imm_expr.X_add_number > OP_MASK_3BITPOS)
		as_bad (_("Bit position for %s not in range 0..7 (%lu)"),
			 ip->insn_mo->name,
			 (unsigned long) imm_expr.X_add_number);
	      INSERT_OPERAND (3BITPOS, *ip, imm_expr.X_add_number);
	      imm_expr.X_op = O_absent;
	      s = expr_end;
	      continue;
...
do_msbd:
	 ...
	 * to figure out where it goes in the instruction.  */
		...
		/* If this value won't fit into a 12 bit offset, then go
		   find a macro that will generate the 32 bit offset
		   code pattern.  */
		i = my_getSmallExpression (&imm_expr, imm_reloc, s);
		if ((i == 0 && (imm_expr.X_op != O_constant
				|| imm_expr.X_add_number >= 0x800
				|| imm_expr.X_add_number < -0x800))
		    || i > 0)
		  {
		    imm_expr.X_op = O_absent;
		...
		INSERT_OPERAND (OFFSET12, *ip, imm_expr.X_add_number);
		imm_expr.X_op = O_absent;
		s = expr_end;
		continue;
	      }
...
{
  {"%lo", BFD_RELOC_MICROMIPS_LO16},
#ifdef OBJ_ELF
  {"%call_hi", BFD_RELOC_MICROMIPS_CALL_HI16},
  {"%call_lo", BFD_RELOC_MICROMIPS_CALL_LO16},
  {"%call16", BFD_RELOC_MICROMIPS_CALL16},
  {"%got_disp", BFD_RELOC_MICROMIPS_GOT_DISP},
  {"%got_page", BFD_RELOC_MICROMIPS_GOT_PAGE},
  {"%got_ofst", BFD_RELOC_MICROMIPS_GOT_OFST},
  {"%got_hi", BFD_RELOC_MICROMIPS_GOT_HI16},
  {"%got_lo", BFD_RELOC_MICROMIPS_GOT_LO16},
  {"%got", BFD_RELOC_MICROMIPS_GOT16},
  {"%gp_rel", BFD_RELOC_MICROMIPS_GPREL16},
  {"%half", BFD_RELOC_MICROMIPS_16},
  {"%highest", BFD_RELOC_MICROMIPS_HIGHEST},
  {"%higher", BFD_RELOC_MICROMIPS_HIGHER},
  {"%neg", BFD_RELOC_MICROMIPS_SUB},
  {"%tlsgd", BFD_RELOC_MICROMIPS_TLS_GD},
  {"%tlsldm", BFD_RELOC_MICROMIPS_TLS_LDM},
  {"%dtprel_hi", BFD_RELOC_MICROMIPS_TLS_DTPREL_HI16},
  {"%dtprel_lo", BFD_RELOC_MICROMIPS_TLS_DTPREL_LO16},
  {"%tprel_hi", BFD_RELOC_MICROMIPS_TLS_TPREL_HI16},
  {"%tprel_lo", BFD_RELOC_MICROMIPS_TLS_TPREL_LO16},
  {"%gottprel", BFD_RELOC_MICROMIPS_TLS_GOTTPREL},
#endif
  {"%hi", BFD_RELOC_MICROMIPS_HI16_S}
};
...
my_getExpression (expressionS *ep, char *str)
{
  ...
  /* If we are in mips16/micromips mode, and this is an expression based
     on `.', then we bump the value of the symbol by 1 since that is how
     other text symbols are handled.  We don't bother to handle complex
...
mips_after_parse_args (void)
{
  ...
  const struct mips_cpu_info *tune_info ATTRIBUTE_UNUSED = 0;
  ...
  mips_set_option_string (&mips_arch_string, "pic32mx");
#else
  ...
  if (mips_opts.ase_mcu && !ISA_SUPPORTS_MCU_ASE)
      as_warn ("%s ISA does not support MCU ASE",
	       mips_cpu_info_from_isa (mips_opts.isa)->name);
...
md_pcrel_from (fixS *fixP)
{
  ...
  switch (fixP->fx_r_type)
    ...
    /* We don't add addr, because it will cause the error checking of
       "addnumber" fail in write.c for *7/10_PCREL_S1.  In tc_gen_reloc,
       we just use fixp->fx_addnumber.  */
    case BFD_RELOC_MICROMIPS_7_PCREL_S1:
    case BFD_RELOC_MICROMIPS_10_PCREL_S1:
...
s_cpload (int ignore ATTRIBUTE_UNUSED)
{
  ...
  macro_build (&ex, "addiu", "t,r,j", mips_gp_register,
	       mips_gp_register, A_BFD_RELOC_LO16);
  if (in_shared)
  ...
  if (mips_in_shared || HAVE_64BIT_SYMBOLS)
    {
      macro_build (&ex_sym, "lui", "t,u", mips_gp_register,
		   -1, A_BFD_RELOC_GPREL16, A_BFD_RELOC_MIPS_SUB,
		   A_BFD_RELOC_HI16_S);

      macro_build (&ex_sym, "addiu", "t,r,j", mips_gp_register,
		   mips_gp_register, -1, A_BFD_RELOC_GPREL16,
		   A_BFD_RELOC_MIPS_SUB, A_BFD_RELOC_LO16);
  ...
  if (mips_cpreturn_register == -1)
    {
      ...
      macro_build (&ex, "ld", "t,o(b)", mips_gp_register, A_BFD_RELOC_LO16, SP);
    }
  else
...
relaxed_micromips_32bit_branch_length (fragS *fragp, asection *sec, int update)
{
  ...
  if (fragp && update && toofar != RELAX_MICROMIPS_TOOFAR (fragp->fr_subtype))
    fragp->fr_subtype
...
relaxed_micromips_16bit_branch_length (fragS *fragp, asection *sec, int update)
{
  ...
  if (RELAX_MICROMIPS_USER_16BIT (fragp->fr_subtype))
    return 2;
...
md_estimate_size_before_relax (fragS *fragp, asection *segtype)
{
  ...
  if (RELAX_MICROMIPS_P (fragp->fr_subtype))
	...
	fragp->fr_var = relaxed_micromips_32bit_branch_length (fragp, segtype,
							       FALSE);
      else
	fragp->fr_var = relaxed_micromips_16bit_branch_length (fragp, segtype,
							       FALSE);
  ...
  if (RELAX_MICROMIPS_P (fragp->fr_subtype))
    {
      ...
      if (RELAX_MICROMIPS_TYPE (fragp->fr_subtype) == 0)
	fragp->fr_var = relaxed_micromips_32bit_branch_length (fragp, sec,
							       TRUE);
      else
	fragp->fr_var = relaxed_micromips_16bit_branch_length (fragp, sec,
							       TRUE);
      return fragp->fr_var - old_var;
    }
...
md_convert_frag (bfd *abfd ATTRIBUTE_UNUSED, segT asec, fragS *fragp)
{
  ...
  /* For relaxing 16-bit microMIPS branches to 32-bit microMIPS branches.  */
  if (RELAX_MICROMIPS_P (fragp->fr_subtype)
	...
	--val;
      ...
      if (RELAX_MICROMIPS_TOOFAR (fragp->fr_subtype))
	{
	  /* We generate a fixup instead of applying it right now
	     because, if there are linker relaxations, we're going to
	     need the relocations.  */
	  ...
	  if ((insn & 0xfc00) == 0xcc00)	/* 16-bit b */
	    insn = 0x94000000;	/* beq */
	  else if ((insn & 0xfc00) == 0x8c00)	/* 16-bit beqz */
	    {
	      int regno = (insn >> MICROMIPSOP_SH_MD) & MICROMIPSOP_MASK_MD;
	      regno = micromips_to_32_reg_d_map [regno];
	      insn = 0x94000000 | (regno << 16);	/* beq */
	    }
	  else if ((insn & 0xfc00) == 0xac00)	/* 16-bit bnez */
	    {
	      int regno = (insn >> MICROMIPSOP_SH_MD) & MICROMIPSOP_MASK_MD;
	      regno = micromips_to_32_reg_d_map [regno];
	      insn = 0xb4000000 | (regno << 16);	/* 32-bit bne */
	    }
	  else
	    abort ();
  ...
  /* For relaxing 32-bit micromips branch to a sequence of instructions.  */
  if (RELAX_MICROMIPS_P (fragp->fr_subtype)
      && RELAX_MICROMIPS_TYPE (fragp->fr_subtype) == 0)
    {
      bfd_byte *buf;
      unsigned long insn, next;
      expressionS exp;
      fixS *fixp;
      ...
      else
	{
	  ...
	  /* Branch from the next instruction by subtracting 4 bytes.  */
	  i -= 4;
	  /* Convert to offset by shifting right 1 bit.  */
	  i >>= 1;
	  insn |= i;
	  /* Branch over the jump.  */
	  md_number_to_chars ((char *) buf, insn >> 16, 2);
	...
	uncond_micromips:
	  if (mips_pic == NO_PIC)
	    {
	      /* j or jal.  */
	      insn = (RELAX_MICROMIPS_LINK (fragp->fr_subtype)
		      ? 0xf4000000 : 0xd4000000);
	      exp.X_op = O_symbol;
	      exp.X_add_symbol = fragp->fr_symbol;
	      exp.X_add_number = fragp->fr_offset;
  ...
  for (i = 0; i < count; i++, syms++)
    {
      if ((ELF_ST_IS_MIPS16 (elf_symbol (*syms)->internal_elf_sym.st_other)
	   || ELF_ST_IS_MICROMIPS (elf_symbol (*syms)->internal_elf_sym.st_other))
	  && ((*syms)->value & 1) != 0)
	{
	  (*syms)->value &= ~1;
...
/* This routine assembles an instruction into its binary format when
   assembling for the microMIPS.  As a side effect, it sets one of the
   global variables imm_reloc or offset_reloc to the type of
   relocation to do if one of the operands is an address expression.  */

static void
micromips_ip (char *str, struct mips_cl_insn *ip)
{
  char *s, *save_s, *t = 0;
  const char *args;
  char c = 0;
  ...
  /* We first try to match an instruction up to space or to the end.  */
  for (s = str; *s != '\0' && !ISSPACE (*s); ++s)
    continue;
  ...
  insn = (struct mips_opcode *) hash_find (micromips_op_hash, str);
  ...
  for (;;)
      ...
      for (args = insn->args;; ++args)
	{
	  ...
	  switch (*args)
		...
		case 'r':
		case 'v':
		  MICROMIPS_INSERT_OPERAND (RS, *ip, lastregno);
		  continue;

		case 'w':
		  MICROMIPS_INSERT_OPERAND (RT, *ip, lastregno);
		  continue;
...
do_lsb:
		  ...
		  lastpos = imm_expr.X_add_number;
		  MICROMIPS_INSERT_OPERAND (EXTLSB, *ip, imm_expr.X_add_number);
		  imm_expr.X_op = O_absent;
		  s = expr_end;
		  continue;
...
do_msb:
		  ...
		  MICROMIPS_INSERT_OPERAND (INSMSB, *ip,
					    lastpos + imm_expr.X_add_number
					    - 1);
		  imm_expr.X_op = O_absent;
		  s = expr_end;
...
do_msbd:
		  ...
		  MICROMIPS_INSERT_OPERAND (EXTMSBD, *ip,
					    imm_expr.X_add_number - 1);
		  imm_expr.X_op = O_absent;
		  s = expr_end;
		  continue;
		...
		default:
		  as_bad (_("internal: bad microMIPS opcode (unknown extension operand type `+%c'): %s %s"),
		    *args, insn->name, insn->args);
		  /* Further processing is fruitless.  */
		  return;
		}
		...
		/* If this value won't fit into a 10 bit offset, then go
		   find a macro that will generate the 32 bit offset
		   code pattern.  */
		i = my_getSmallExpression (&imm_expr, imm_reloc, s);
		if ((i == 0 && (imm_expr.X_op != O_constant
		...
		MICROMIPS_INSERT_OPERAND (OFFSET10, *ip, imm_expr.X_add_number);
		imm_expr.X_op = O_absent;
		s = expr_end;
		continue;
	      }

	    case '1':		/* 5-bit sync type */
	    case 'h':		/* prefx code */
	      my_getExpression (&imm_expr, s);
	      check_absolute_expr (ip, &imm_expr);
	      if ((unsigned long) imm_expr.X_add_number > 31)
		as_bad (_("Invalid value for `%s' (%lu)"),
			ip->insn_mo->name,
			(unsigned long) imm_expr.X_add_number);
	      if (*args == 'h')
		MICROMIPS_INSERT_OPERAND (PREFX, *ip, imm_expr.X_add_number);
	      else
		MICROMIPS_INSERT_OPERAND (STYPE, *ip, imm_expr.X_add_number);
	      imm_expr.X_op = O_absent;
	      s = expr_end;
	      continue;
		...
		as_warn (_("Improper shift amount (%lu)"),
			 (unsigned long) imm_expr.X_add_number);
	      MICROMIPS_INSERT_OPERAND (SHAMT, *ip, imm_expr.X_add_number);
	      imm_expr.X_op = O_absent;
	      s = expr_end;
	      continue;
	    ...
	    case '>':		/* shift amount minus 32 */
		  ...
		  || (unsigned long) imm_expr.X_add_number > 63)
		break;
	      MICROMIPS_INSERT_OPERAND (SHAMT, *ip, imm_expr.X_add_number - 32);
	      imm_expr.X_op = O_absent;
	      s = expr_end;
	      continue;
		...
		as_bad (_("Bit position for %s not in range 0..7 (%lu)"),
			 ip->insn_mo->name,
			 (unsigned long) imm_expr.X_add_number);
	      MICROMIPS_INSERT_OPERAND (3BITPOS, *ip, imm_expr.X_add_number);
	      imm_expr.X_op = O_absent;
	      s = expr_end;
	      continue;

	    case '|':		/* 10-bit lower break code */
	      my_getExpression (&imm_expr, s);
	      check_absolute_expr (ip, &imm_expr);
	      if ((unsigned long) imm_expr.X_add_number
		  > MICROMIPSOP_MASK_BREAKCODE)
		as_bad (_("Code for %s not in range 0..1023 (%lu)"),
			ip->insn_mo->name,
			(unsigned long) imm_expr.X_add_number);
	      MICROMIPS_INSERT_OPERAND (BREAKCODE2, *ip, imm_expr.X_add_number);
	      imm_expr.X_op = O_absent;
	      s = expr_end;
	      continue;
		...
		MICROMIPS_INSERT_OPERAND (OFFSET12, *ip, imm_expr.X_add_number);
		imm_expr.X_op = O_absent;
		s = expr_end;
		continue;
	      }
	    ...
	    case 'B':		/* 10-bit syscall/break/wait code.  */
	      my_getExpression (&imm_expr, s);
	      check_absolute_expr (ip, &imm_expr);
	      if ((unsigned long) imm_expr.X_add_number
		...
		as_bad (_("Code for %s not in range 0..1023 (%lu)"),
			ip->insn_mo->name,
			(unsigned long) imm_expr.X_add_number);
	      MICROMIPS_INSERT_OPERAND (CODE10, *ip, imm_expr.X_add_number);
	      imm_expr.X_op = O_absent;
	      s = expr_end;
	      continue;

	    case 'C':		/* 23-bit Coprocessor code */
	      my_getExpression (&imm_expr, s);
	      check_absolute_expr (ip, &imm_expr);
	      if ((unsigned long) imm_expr.X_add_number > MICROMIPSOP_MASK_COPZ)
		{
		  as_warn (_("Coproccesor code > 23 bits (%lu)"),
			   (unsigned long) imm_expr.X_add_number);
		  imm_expr.X_add_number &= MICROMIPSOP_MASK_COPZ;
		}
	      MICROMIPS_INSERT_OPERAND (COPZ, *ip, imm_expr.X_add_number);
	      imm_expr.X_op = O_absent;
	      s = expr_end;
	      continue;
	    ...
	    case 'G':		/* Coprocessor destination register.  */
	      if ((ip->insn_opcode & 0xfc001fff) == 0x000000fc /* mfc0 */
		  || (ip->insn_opcode & 0xfc001fff) == 0x000002fc /* mtc0 */
		  || (ip->insn_opcode & 0xfc001fff) == 0x58000003 /* dmfc0 */
		  || (ip->insn_opcode & 0xfc001fff) == 0x58000043) /* dmtc0 */
		ok = reg_lookup (&s, RTYPE_NUM | RTYPE_CP0, &regno);
	      else
		ok = reg_lookup (&s, RTYPE_NUM | RTYPE_GP, &regno);
	      MICROMIPS_INSERT_OPERAND (RS, *ip, regno);
	      if (ok)
		{
		  lastregno = regno;
		  continue;
		...
		c = 8; /* Invalid sel value.  */
	      ...
	      if (c > 7)
		as_bad (_("invalid coprocessor sub-selection value (0-7)"));
	      MICROMIPS_INSERT_OPERAND (COPSEL, *ip, c);
	      continue;
	    ...
	    case 'N':		/* 3 bit branch condition code */
	      rtype = RTYPE_CCC;
	      if ((ip->insn_mo->pinfo & (FP_D| FP_S))
		  && strncmp (ip->insn_mo->name, "bc2", 3) != 0)
		rtype |= RTYPE_FCC;
	      if (!reg_lookup (&s, rtype, &regno))
		break;
	      if ((strcmp(str + strlen(str) - 3, ".ps") == 0
		...
		as_warn(_("Condition code register should be 0 or 4 for %s, was %d"),
			str, regno);
	      if (*args == 'N')
		MICROMIPS_INSERT_OPERAND (BCC, *ip, regno);
	      else
		MICROMIPS_INSERT_OPERAND (CCC, *ip, regno);
	      continue;

	    case 'a':		/* 26 bit address */
	      my_getExpression (&offset_expr, s);
	      s = expr_end;
	      *offset_reloc = BFD_RELOC_MICROMIPS_JMP;
	      continue;
	    ...
	    case 'K':		/* 'rdhwr' destination register */
	    case 'z':		/* must be zero register */
	    case 'J':		/* source register for alnv.ps */
	      s_reset = s;
	      if (*args == 'E' || *args == 'K')
		ok = reg_lookup (&s, RTYPE_NUM, &regno);
	      else
	 ...
	 * to figure out where it goes in the instruction.  */
		  switch (c)
		    ...
		    case 'J':
		      MICROMIPS_INSERT_OPERAND (RS3, *ip, regno);
		      break;

		    case 'd':
		      MICROMIPS_INSERT_OPERAND (RD, *ip, regno);
		      break;

		    case 'w':
		    case 't':
		    case 'E':
		      MICROMIPS_INSERT_OPERAND (RT, *ip, regno);
		      break;
	    ...
	    case 'c':		/* 10-bit higher break code */
	      my_getExpression (&imm_expr, s);
	      check_absolute_expr (ip, &imm_expr);
	      if ((unsigned long) imm_expr.X_add_number
		  > MICROMIPSOP_MASK_BREAKCODE)
		as_bad (_("Code for %s not in range 0..1023 (%lu)"),
			ip->insn_mo->name,
			(unsigned long) imm_expr.X_add_number);
	      MICROMIPS_INSERT_OPERAND (BREAKCODE, *ip, imm_expr.X_add_number);
	      imm_expr.X_op = O_absent;
	      s = expr_end;
	      continue;
	    ...
	    case 'i':		/* 16 bit unsigned immediate */
	    case 'j':		/* 16 bit signed immediate */
	      *imm_reloc = BFD_RELOC_MICROMIPS_LO16;
	      if (my_getSmallExpression (&imm_expr, imm_reloc, s) == 0)
		{
		  int more;
		  offsetT minval, maxval;
		...
		as_bad (_("Invalid value for `%s' (%lu)"),
			ip->insn_mo->name,
			(unsigned long) imm_expr.X_add_number);
	      MICROMIPS_INSERT_OPERAND (CACHEOP, *ip, imm_expr.X_add_number);
	      imm_expr.X_op = O_absent;
	      s = expr_end;
	      continue;
		...
		while (1)
		  {
			...
			/* Set all registers from reg1 + 1 to regno.  */
			if (regno == 30) /* s8 */
			  {
			    reg_list |= (1 << regno);
			    regno = 23; /* s7 */
			  }
			for (i = reg1 + 1; i <= regno; i++)
			  reg_list |= (1 << i);
		      }
		    else
		      reg_list |= (1 << regno);
		...
		MICROMIPS_INSERT_OPERAND (RT, *ip, immed);
		imm_expr.X_op = O_absent;
		continue;
	      }
	    ...
	    case 'p':		/* pc relative offset */
	      *offset_reloc = BFD_RELOC_MICROMIPS_16_PCREL_S1;
	      my_getExpression (&offset_expr, s);
	      s = expr_end;
	      continue;

	    case 'q':		/* 4-bit trap code.  */
	      my_getExpression (&imm_expr, s);
	      check_absolute_expr (ip, &imm_expr);
	      if ((unsigned long) imm_expr.X_add_number > MICROMIPSOP_MASK_TRAP)
		as_bad (_("Trap code for %s not in range 0..15 (%lu)"),
			 ip->insn_mo->name,
			 (unsigned long) imm_expr.X_add_number);
	      MICROMIPS_INSERT_OPERAND (TRAP, *ip, imm_expr.X_add_number);
	      imm_expr.X_op = O_absent;
	      s = expr_end;
	      continue;
		...
		case 'z':
		  ...
		  if (!ok)
			...
			{
			  case 'c':
			    regno = mips32_to_micromips_reg_c_map[lastregno];
			    if (regno == ILLEGAL_REG)
			      break;
			    MICROMIPS_INSERT_OPERAND (MC, *ip, regno);
			    continue;
		  ...
		  switch (c)
			...
			break;
		      ...
		      case 'i':
			switch (MICROMIPS_EXTRACT_OPERAND (MI, *ip))
			  {
			    case  4:
			      if (regno == 21)
				regno = 3;
		  ...
		  switch (c)
		    {
		      case 'b':
			MICROMIPS_INSERT_OPERAND (MB, *ip, regno);
			break;
			...
			break;
		      ...
		      case 'd':
			MICROMIPS_INSERT_OPERAND (MD, *ip, regno);
			break;

		      case 'e':
			MICROMIPS_INSERT_OPERAND (ME, *ip, regno);
			break;

		      case 'f':
			MICROMIPS_INSERT_OPERAND (MF, *ip, regno);
			break;

		      case 'g':
			MICROMIPS_INSERT_OPERAND (MG, *ip, regno);
			break;

		      case 'h':
			MICROMIPS_INSERT_OPERAND (MH, *ip, regno);
			break;

		      case 'i':
			MICROMIPS_INSERT_OPERAND (MI, *ip, regno);
			break;

		      case 'j':
			MICROMIPS_INSERT_OPERAND (MJ, *ip, regno);
			break;

		      case 'l':
			MICROMIPS_INSERT_OPERAND (ML, *ip, regno);
			break;

		      case 'm':
			MICROMIPS_INSERT_OPERAND (MM, *ip, regno);
			break;

		      case 'n':
			MICROMIPS_INSERT_OPERAND (MN, *ip, regno);
			break;

		      case 'p':
			MICROMIPS_INSERT_OPERAND (MP, *ip, regno);
			break;

		      case 'q':
			MICROMIPS_INSERT_OPERAND (MQ, *ip, regno);
			break;
		...
		case 'A':
		  {
		    ...
		    if (*s == '(' && strchr (s + 1, '(') == 0)
		      {
			MICROMIPS_INSERT_OPERAND (IMMA, *ip, 0);
			continue;
		      }
		    ...
		    MICROMIPS_INSERT_OPERAND (IMMA, *ip, immed);
		    imm_expr.X_op = O_absent;
		    s = expr_end;
		    continue;
		  }
		...
		case 'B':
		  {
		    ...
		    MICROMIPS_INSERT_OPERAND (IMMB, *ip, immb);
		    imm_expr.X_op = O_absent;
		    s = expr_end;
		    continue;
		  }
		...
		case 'C':
		  {
		    ...
		    for (immc = 0; immc < 16; immc++)
		      {
			if (micromips_imm_c_map[immc]
			    == imm_expr.X_add_number)
			  break;
		      }
		    if ((i == 0 && (imm_expr.X_op != O_constant || immc == 16))
			|| i > 0)
		      {
			imm_expr.X_op = O_absent;
			break;
		      }
		    MICROMIPS_INSERT_OPERAND (IMMC, *ip, immc);
		    imm_expr.X_op = O_absent;
		    s = expr_end;
		    continue;
		  }
		...
		case 'E':	/* pc relative offset */
		  ...
		  if (mips_relax_branch && !micromips_16)
		    break;
		...
		case 'F':
		  {
		    ...
		    immed = imm_expr.X_add_number;
		    MICROMIPS_INSERT_OPERAND (IMMF, *ip, immed);
		    imm_expr.X_op = O_absent;
		    s = expr_end;
		    continue;
		  }
		...
		case 'G':
		  {
		    ...
		    if (*s == '(' && strchr (s + 1, '(') == 0)
		      {
			MICROMIPS_INSERT_OPERAND (IMMG, *ip, 0);
			continue;
		      }
		    ...
		    if (immed == -1)
		      immed = 15;
		    MICROMIPS_INSERT_OPERAND (IMMG, *ip, immed);
		    imm_expr.X_op = O_absent;
		    s = expr_end;
		    continue;
		  }
		...
		case 'H':
		  {
		    ...
		    if (*s == '(' && strchr (s + 1, '(') == 0)
		      {
			MICROMIPS_INSERT_OPERAND (IMMH, *ip, 0);
			continue;
		      }

		    i = my_getSmallExpression (&imm_expr, imm_reloc, s);
		    if ((i == 0 && (imm_expr.X_op != O_constant
				    || (imm_expr.X_add_number & 1) != 0
				    || imm_expr.X_add_number > (15 << 1)
				    || imm_expr.X_add_number < 0))
			|| i > 0)
		      {
			imm_expr.X_op = O_absent;
		    ...
		    immed = imm_expr.X_add_number >> 1;
		    MICROMIPS_INSERT_OPERAND (IMMH, *ip, immed);
		    imm_expr.X_op = O_absent;
		    s = expr_end;
		    continue;
		  }
		...
		case 'I':
		  {
		    ...
		    if (immed == -1)
		      immed = 127;
		    MICROMIPS_INSERT_OPERAND (IMMI, *ip, immed);
		    imm_expr.X_op = O_absent;
		    s = expr_end;
		    continue;
		  }
		...
		case 'J':
		  {
		    ...
		    if (*s == '(' && strchr (s + 1, '(') == 0)
		      {
			MICROMIPS_INSERT_OPERAND (IMMJ, *ip, 0);
			continue;
		      }
		    ...
		    immed = imm_expr.X_add_number >> 2;
		    MICROMIPS_INSERT_OPERAND (IMMJ, *ip, immed);
		    imm_expr.X_op = O_absent;
		    s = expr_end;
		    continue;
		  }
		...
		case 'L':
		  {
		    ...
		    if (*s == '(' && strchr (s + 1, '(') == 0)
		      {
			MICROMIPS_INSERT_OPERAND (IMML, *ip, 0);
			continue;
		      }
		    ...
		    immed = imm_expr.X_add_number;
		    MICROMIPS_INSERT_OPERAND (IMML, *ip, immed);
		    imm_expr.X_op = O_absent;
		    s = expr_end;
		    continue;
		  }
		...
		case 'M':
		  {
		    ...
		    if (immed == 8)
		      immed = 0;
		    MICROMIPS_INSERT_OPERAND (IMMM, *ip, immed);
		    imm_expr.X_op = O_absent;
		    s = expr_end;
		    continue;
		  }
		...
		case 'N':	/* Register list for lwm and swm.  */
		  {
		    ...
		    MICROMIPS_INSERT_OPERAND (IMMN, *ip, immed);
		    imm_expr.X_op = O_absent;
		    continue;
		  }
		...
		case 'O':	/* sdbbp 4-bit code.  */
		  {
		    ...
		    immed = imm_expr.X_add_number;
		    MICROMIPS_INSERT_OPERAND (IMMO, *ip, immed);
		    imm_expr.X_op = O_absent;
		    s = expr_end;
		    continue;
		  }
		...
		case 'P':
		  {
		    ...
		    immed = (imm_expr.X_add_number >> 2);
		    MICROMIPS_INSERT_OPERAND (IMMP, *ip, immed);
		    imm_expr.X_op = O_absent;
		    s = expr_end;
		    continue;
		  }
		...
		case 'Q':
		  {
			...
			&& imm_expr.X_add_number <= (4194303 << 2))
		      {
			immed = imm_expr.X_add_number >> 2;
		        MICROMIPS_INSERT_OPERAND (IMMQ, *ip, immed);
		      }
		    else
		      {
			imm_expr.X_op = O_absent;
		...
		case 'U':
		  {
		    ...
		    if (*s == '(' && strchr (s + 1, '(') == 0)
		      {
			MICROMIPS_INSERT_OPERAND (IMMU, *ip, 0);
			continue;
		      }
		    ...
		    immed = imm_expr.X_add_number >> 2;
		    MICROMIPS_INSERT_OPERAND (IMMU, *ip, immed);
		    imm_expr.X_op = O_absent;
		    s = expr_end;
		    continue;
		  }
		...
		case 'W':
		  {
			...
			&& imm_expr.X_add_number <= (63 << 2))
		      {
			immed = imm_expr.X_add_number >> 2;
			MICROMIPS_INSERT_OPERAND (IMMW, *ip, immed);
		      }
		    else
		      {
			imm_expr.X_op = O_absent;
		...
		case 'X':
		  {
			...
			&& imm_expr.X_add_number <= 7)
		      {
			immed = imm_expr.X_add_number;
			MICROMIPS_INSERT_OPERAND (IMMX, *ip, immed);
		      }
		    else
		      {
			imm_expr.X_op = O_absent;
		...
		case 'Y':
		  {
		    ...
		    if (i == 0 && imm_expr.X_op == O_constant
			&& (imm_expr.X_add_number & 3) == 0
			&& imm_expr.X_add_number >= (-258 << 2)
			&& imm_expr.X_add_number != (-2 << 2)
			&& imm_expr.X_add_number != (-1 << 2)
			&& imm_expr.X_add_number != (0 << 2)
			&& imm_expr.X_add_number != (1 << 2)
			&& imm_expr.X_add_number <= (257 << 2))
		      {
			if (imm_expr.X_add_number == (-258 << 2))
			  imm_expr.X_add_number = (-2 << 2);
			if (imm_expr.X_add_number == (-257 << 2))
			  imm_expr.X_add_number = (-1 << 2);
			if (imm_expr.X_add_number == (256 << 2))
			  imm_expr.X_add_number = (0 << 2);
			if (imm_expr.X_add_number == (257 << 2))
			  imm_expr.X_add_number = (1 << 2);
			immed = imm_expr.X_add_number >> 2;
			MICROMIPS_INSERT_OPERAND (IMMY, *ip, immed);
		      }
		    else
		      {
			imm_expr.X_op = O_absent;
		...
		default:
		  as_bad (_("internal: bad microMIPS opcode (unknown extension operand type `m%c'): %s %s"),
		    *args, insn->name, insn->args);
		  /* Further processing is fruitless.  */
		  return;
		}
...
static void
micromips_macro_build (expressionS *ep, const char *name, const char *fmt,
		       va_list *args)
{
  const struct mips_opcode *mo;
  struct mips_cl_insn insn;
  ...
  mo = (struct mips_opcode *) hash_find (micromips_op_hash, name);
  gas_assert (mo);
  gas_assert (strcmp (name, mo->name) == 0);
  ...
  for (;;)
	...
	case '+':
	  switch (*fmt++)
	    ...
	    case 'A':
	    case 'E':
	      MICROMIPS_INSERT_OPERAND (EXTLSB, insn, va_arg (*args, int));
	      continue;
	    ...
	    case 'F':
		 ...
		 non-macro case, these arguments are sizes from which
		 MSB values must be calculated.)  */
	      MICROMIPS_INSERT_OPERAND (INSMSB, insn, va_arg (*args, int));
	      continue;
	    ...
	    case 'H':
		 ...
		 non-macro case, these arguments are sizes from which
		 MSBD values must be calculated.)  */
	      MICROMIPS_INSERT_OPERAND (EXTMSBD, insn, va_arg (*args, int));
	      continue;
	...
	case 'n':
	case 'E':
	  MICROMIPS_INSERT_OPERAND (RT, insn, va_arg (*args, int));
	  continue;

	case 'T':
	  MICROMIPS_INSERT_OPERAND (FT, insn, va_arg (*args, int));
	  continue;

	case 'd':
	  MICROMIPS_INSERT_OPERAND (RD, insn, va_arg (*args, int));
	  continue;
	...
	case '<':
	  MICROMIPS_INSERT_OPERAND (SHAMT, insn, va_arg (*args, int));
	  continue;
	...
	case 'v':
	case 'G':
	  MICROMIPS_INSERT_OPERAND (RS, insn, va_arg (*args, int));
	  continue;
	...
	case 'u':
	  ...
	  gas_assert (ep != NULL
		  && (ep->X_op == O_constant
			  ...
			  && (*r == BFD_RELOC_MIPS_HIGHEST
			      || *r == BFD_RELOC_MICROMIPS_HI16_S
			      || *r == BFD_RELOC_MICROMIPS_HI16
			      || *r == BFD_RELOC_MICROMIPS_GPREL16
			      || *r == BFD_RELOC_MICROMIPS_GOT_HI16
			      || *r == BFD_RELOC_MICROMIPS_CALL_HI16))));
	  continue;
	...
	case 'a':
	  gas_assert (ep != NULL);
	  *r = BFD_RELOC_MICROMIPS_JMP;
	  continue;

	case 'k':
	  MICROMIPS_INSERT_OPERAND (CACHEOP, insn,
				    va_arg (*args, unsigned long));
	  continue;

	case '\\':
	  MICROMIPS_INSERT_OPERAND (3BITPOS, insn, va_arg (*args, int));
	  continue;

	case 'q':
	  MICROMIPS_INSERT_OPERAND (TRAP, insn, va_arg (*args, int));
	  continue;

	case '.':
	  MICROMIPS_INSERT_OPERAND (OFFSET10, insn, va_arg (*args, int));
	  continue;

	case 'J':
	case '~':
	  MICROMIPS_INSERT_OPERAND (OFFSET12, insn,
				    va_arg (*args, unsigned long));
	  continue;

	case 'N':
	  MICROMIPS_INSERT_OPERAND (BCC, insn, va_arg (*args, int));
	  continue;
	...
	case 'm':	/* Opcode extension character.  */
	  switch (*fmt++)
	    {
	    case 'j':
	      MICROMIPS_INSERT_OPERAND (MJ, insn, va_arg (*args, int));
	      break;

	    case 'p':
	      MICROMIPS_INSERT_OPERAND (MP, insn, va_arg (*args, int));
	      break;

	    case 'F':
	      MICROMIPS_INSERT_OPERAND (IMMF, insn, va_arg (*args, int));
	      break;
...
/* Implement macros in microMIPS mode.  */

static void
micromips_macro (struct mips_cl_insn *ip)
{
  unsigned int treg, sreg, dreg, breg;
  unsigned int tempreg;
  int used_at = 0;
  ...
  treg = MICROMIPS_EXTRACT_OPERAND (RT, *ip);
  dreg = MICROMIPS_EXTRACT_OPERAND (RD, *ip);
  sreg = breg = MICROMIPS_EXTRACT_OPERAND (RS, *ip);
  mask = ip->insn_mo->mask;
  ...
  switch (mask)
	 ...
	 */
      ...
      my_getExpression (&expr2, MICROMIPS_TARGET);
      macro_build (&expr2, "bgez", "s,p", sreg);
      if (dreg == sreg)
	macro_build (NULL, "nop", "", 0);
      else
	move_register (dreg, sreg);
    ...
    do_addi:
	  ...
	  && imm_expr.X_add_number < 0x8000)
	{
	  macro_build (&imm_expr, s, "t,r,j", treg, sreg,
    ...
    case M_DADD_I:
	  ...
	  && imm_expr.X_add_number < 0x200)
	{
	  macro_build (NULL, "daddi", "t,r,.", treg, sreg,
		       imm_expr.X_add_number);
	  break;
	}
      used_at = 1;
      load_register (AT, &imm_expr, 1);
      macro_build (NULL, "dadd", "d,v,t", treg, sreg, AT);
      break;
	...
	{
	  if (mask != M_NOR_I)
	    macro_build (&imm_expr, s, "t,r,i", treg, sreg,
			 BFD_RELOC_MICROMIPS_LO16);
	  else
	    {
	      macro_build (&imm_expr, "ori", "t,r,i",
			   treg, sreg, BFD_RELOC_MICROMIPS_LO16);
	      macro_build (NULL, "nor", "d,v,t", treg, treg, 0);
	    }
	  break;
	}
      ...
      if (mips_opts.noreorder)
	{
	  /*
	    ...
	    1:
	   */
  	  unsigned int fccreg = MICROMIPS_EXTRACT_OPERAND (BCC, *ip);
	  start_noreorder ();
	  my_getExpression (&expr2, MICROMIPS_TARGET);
	  macro_build (&expr2, s, "N,p", fccreg);
	  macro_build (NULL, "nop", "", 0);
	  macro_build (&offset_expr, "b", "p");
	  end_noreorder ();

	  /* Set to true, so that append_insn may add a label.  */
	  emit_branch_likely_macro = TRUE;
	}
      else
	{
	  /* Just translate it to beq, because the delay slot will be nop.  */
  	  unsigned int fccreg = MICROMIPS_EXTRACT_OPERAND (BCC, *ip);
	  start_noreorder ();
	  macro_build (&offset_expr, s2, "N,p", fccreg);
	  macro_build (NULL, "nop", "", 0);
	  end_noreorder ();
	}
      break;
      ...
      if (mips_opts.noreorder)
	{
	  ...
	  start_noreorder ();
	  my_getExpression (&expr2, MICROMIPS_TARGET);
	  macro_build (&expr2, s, "s,t,p", sreg, treg);
	  macro_build (NULL, "nop", "", 0);
	  macro_build (&offset_expr, "b", "p");
	  end_noreorder ();
      ...
      else
	{
	  /* Just translate it to beq, because the delay slot will be nop.  */
	  start_noreorder ();
	  macro_build (&offset_expr, s2, "s,t,p", sreg, treg);
	  macro_build (NULL, "nop", "", 0);
	  end_noreorder ();
	}
      break;
      ...
      else
	{
	  /* Just translate it to beq, because the delay slot will be nop.  */
	  start_noreorder ();
	  macro_build (&offset_expr, s2, "s,p", sreg);
	  /* For b*al, the delay slot needs to be 32 bits.  */
	  if (proc_call)
	    macro_build (NULL, "nop32", "", 0);
	  else
	    macro_build (NULL, "nop", "", 0);
	  end_noreorder ();
	}
      ...
      if (treg == 0)
	{
	  if (likely)
	    goto do_bgezl;
	  macro_build (&offset_expr, "bgez", "s,p", sreg);
	  break;
	}
      if (sreg == 0)
	{
	  ...
	  macro_build (&offset_expr, "blez", "s,p", treg);
	  break;
	}
      used_at = 1;
      macro_build (NULL, "slt", "d,v,t", AT, sreg, treg);
      if (likely)
	{
      ...
      macro_build (&offset_expr, "beq", "s,t,p", AT, 0);
      break;
      ...
      if (imm_expr.X_op == O_constant && imm_expr.X_add_number == 1)
	{
	  if (likely)
	    goto do_bgtzl;
	  macro_build (&offset_expr, "bgtz", "s,p", sreg);
	  break;
	}
      maxnum = 0x7fffffff;
      if (HAVE_64BIT_GPRS && sizeof (maxnum) > 4)
      ...
      if (sreg == 0)
	{
	  ...
	  macro_build (&offset_expr, "beq", "s,t,p", 0, treg);
	  break;
	}
      used_at = 1;
      macro_build (NULL, "sltu", "d,v,t", AT, sreg, treg);
    ...
    case M_BGTU_I:
	  ...
	  || (HAVE_32BIT_GPRS
	      && imm_expr.X_op == O_constant
	      && imm_expr.X_add_number == (offsetT) 0xffffffffffffffffLL))
	goto do_false;
      if (imm_expr.X_op != O_constant)
	as_bad (_("Unsupported large constant"));
      ++imm_expr.X_add_number;
      ...
      if (imm_expr.X_op == O_constant && imm_expr.X_add_number == 1)
	{
	  ...
	  macro_build (&offset_expr, "bne", "s,t,p", sreg, 0);
	  break;
	}
      used_at = 1;
      set_at (sreg, 1);
      ...
      if (sreg == 0)
	{
	  ...
	  macro_build (&offset_expr, "bltz", "s,p", treg);
	  break;
	}
      used_at = 1;
      macro_build (NULL, "slt", "d,v,t", AT, treg, sreg);
      if (likely)
	{
      ...
      macro_build (&offset_expr, "bne", "s,t,p", AT, 0);
      break;
      ...
      if (treg == 0)
	{
	  if (likely)
	    goto do_blezl;
	  macro_build (&offset_expr, "blez", "s,p", sreg);
	  break;
	}
      if (sreg == 0)
	{
	  ...
	  macro_build (&offset_expr, "bgez", "s,p", treg);
	  break;
	}
      used_at = 1;
      macro_build (NULL, "slt", "d,v,t", AT, treg, sreg);
      ...
      if (imm_expr.X_op == O_constant && imm_expr.X_add_number == 0)
	{
	  if (likely)
	    goto do_bltzl;
	  macro_build (&offset_expr, "bltz", "s,p", sreg);
	  break;
	}
      if (imm_expr.X_op == O_constant && imm_expr.X_add_number == 1)
	{
      ...
      if (treg == 0)
	{
	  if (likely)
	    goto do_beql;
	  macro_build (&offset_expr, "beq", "s,t,p", sreg, 0);
	  break;
	}
      if (sreg == 0)
	goto do_true;
      ...
      if (sreg == 0)
	{
	  ...
	  macro_build (&offset_expr, "bgtz", "s,p", treg);
	  break;
	}
      used_at = 1;
      macro_build (NULL, "slt", "d,v,t", AT, sreg, treg);
      ...
      if (sreg == 0)
	{
	  if (likely)
	    goto do_bnel;
	  macro_build (&offset_expr, "bne", "s,t,p", 0, treg);
	  break;
	}
      used_at = 1;
      macro_build (NULL, "sltu", "d,v,t", AT, sreg, treg);
	...
	{
	  my_getExpression (&expr2, MICROMIPS_TARGET);
	  macro_build (&expr2, "bne", "s,t,p", treg, 0);
	  macro_build (NULL, dbl ? "ddiv" : "div", "z,s,t", sreg, treg);
	  macro_build (NULL, "break", "mF", 7);
	  micromips_add_number_label ();
	}
      ...
      load_register (AT, &expr1, dbl);
      my_getExpression (&expr2, MICROMIPS_TARGET);
      macro_build (&expr2, "bne", "s,t,p", treg, AT);
      if (dbl)
	{
	  expr1.X_add_number = 1;
	  load_register (AT, &expr1, dbl);
	  macro_build (NULL, "dsll32", "t,r,<", AT, AT, 31);
	}
     else
	{
	  expr1.X_add_number = 0x80000000;
	  macro_build (&expr1, "lui", "t,u", AT, BFD_RELOC_MICROMIPS_HI16);
	}
      if (mips_trap)
	{
	  macro_build (NULL, "teq", "s,t,q", sreg, AT, 6);
	...
	{
	  my_getExpression (&expr2, MICROMIPS_TARGET);
	  macro_build (&expr2, "bne", "s,t,p", sreg, AT);
	  macro_build (NULL, "nop", "", 0);
	  ...
	  macro_build (NULL, "break", "mF", 6);
	}
      micromips_add_number_label ();
      macro_build (NULL, s, "mj", dreg);
      break;
	...
	{
	  macro_build (&offset_expr, ADDRESS_ADDI_INSN,
		       "t,r,j", treg, sreg, BFD_RELOC_MICROMIPS_LO16);
	  break;
	}
	...
	{
	  /* If this is a reference to a GP relative symbol, we want
	       addiu	$tempreg,$gp,<sym>	(BFD_RELOC_MICROMIPS_GPREL16)
	     Otherwise we want
	       lui	$tempreg,<sym>		(BFD_RELOC_MICROMIPS_HI16_S)
	       addiu	$tempreg,$tempreg,<sym>	(BFD_RELOC_MICROMIPS_LO16)
	     If we have a constant, we need two instructions anyhow,
	     so we may as well always use the latter form.

	     With 64bit address space and a usable $at we want
	       lui	$tempreg,<sym>		(BFD_RELOC_MICROMIPS_HIGHEST)
	       lui	$at,<sym>		(BFD_RELOC_MICROMIPS_HI16_S)
	       daddiu	$tempreg,<sym>		(BFD_RELOC_MICROMIPS_HIGHER)
	       daddiu	$at,<sym>		(BFD_RELOC_MICROMIPS_LO16)
	       dsll32	$tempreg,0
	       daddu	$tempreg,$tempreg,$at
	     ...
	     on superscalar processors.
	       ...
	       daddiu	$tempreg,<sym>		(BFD_RELOC_MICROMIPS_HIGHER)
	       dsll	$tempreg,16
	       daddiu	$tempreg,<sym>		(BFD_RELOC_MICROMIPS_HI16_S)
	       dsll	$tempreg,16
	       daddiu	$tempreg,<sym>		(BFD_RELOC_MICROMIPS_LO16)
	  ...
	  if (HAVE_64BIT_SYMBOLS)
		...
		{
		  macro_build (&offset_expr, "lui", "t,u",
			       tempreg, BFD_RELOC_MICROMIPS_HIGHEST);
		  macro_build (&offset_expr, "lui", "t,u",
			       AT, BFD_RELOC_MICROMIPS_HI16_S);
		  macro_build (&offset_expr, "daddiu", "t,r,j",
			       tempreg, tempreg, BFD_RELOC_MICROMIPS_HIGHER);
		  macro_build (&offset_expr, "daddiu", "t,r,j",
			       AT, AT, BFD_RELOC_MICROMIPS_LO16);
		  macro_build (NULL, "dsll32", "d,w,<", tempreg, tempreg, 0);
		  macro_build (NULL, "daddu", "d,v,t", tempreg, tempreg, AT);
		  used_at = 1;
		}
	      else
		{
		  ...
		  macro_build (NULL, "dsll", "d,w,<", tempreg, tempreg, 16);
		  macro_build (&offset_expr, "daddiu", "t,r,j",
			       tempreg, tempreg, BFD_RELOC_MICROMIPS_HI16_S);
		  macro_build (NULL, "dsll", "d,w,<", tempreg, tempreg, 16);
		  macro_build (&offset_expr, "daddiu", "t,r,j",
			       tempreg, tempreg, BFD_RELOC_MICROMIPS_LO16);
		}
	...
	{
	  int lw_reloc_type = (int) BFD_RELOC_MICROMIPS_GOT16;

	  /* If this is a reference to an external symbol, and there
	     is no constant, we want
	       lw	$tempreg,<sym>($gp)	(BFD_RELOC_MICROMIPS_GOT16)
	     or for lca or if tempreg is PIC_CALL_REG
	       lw	$tempreg,<sym>($gp)	(BFD_RELOC_MICROMIPS_CALL16)
	     For a local symbol, we want
	       lw	$tempreg,<sym>($gp)	(BFD_RELOC_MICROMIPS_GOT16)
	       nop
	       addiu	$tempreg,$tempreg,<sym>	(BFD_RELOC_MICROMIPS_LO16)
	     ...
	     an external symbol, we want
	       ...
	       addiu	$tempreg,$tempreg,<constant>
	     For a local symbol, we want the same instruction
	     sequence, but we output a BFD_RELOC_MICROMIPS_LO16 reloc on the
	     addiu instruction.
	  ...
	  if (offset_expr.X_add_number == 0)
	    {
		  ...
		  && breg == 0
		  && (call || tempreg == PIC_CALL_REG))
		lw_reloc_type = (int) BFD_RELOC_MICROMIPS_CALL16;
	      ...
	      if (breg != 0)
		{
	      ...
	      relax_switch ();
	      macro_build (&offset_expr, ADDRESS_LOAD_INSN, "t,o(b)",
			   tempreg, BFD_RELOC_MICROMIPS_GOT16,
			   mips_gp_register);
	      load_delay_nop ();
	      macro_build (&offset_expr, ADDRESS_ADDI_INSN, "t,r,j",
			   tempreg, tempreg, BFD_RELOC_MICROMIPS_LO16);
      ...
      else if (!mips_big_got && HAVE_NEWABI)
	{
	  ...
	  /* If this is a reference to an external, and there is no
	     constant, or local symbol (*), with or without a
	     constant, we want
	       lw	$tempreg,<sym>($gp)	(BFD_RELOC_MICROMIPS_GOT_DISP)
	     or for lca or if tempreg is PIC_CALL_REG
	       lw	$tempreg,<sym>($gp)	(BFD_RELOC_MICROMIPS_CALL16)
	  ...
	  if (offset_expr.X_add_number)
	    {
	      ...
	      relax_start (offset_expr.X_add_symbol);
	      macro_build (&offset_expr, ADDRESS_LOAD_INSN, "t,o(b)", tempreg,
			   BFD_RELOC_MICROMIPS_GOT_DISP, mips_gp_register);
	  ...
	  else if (breg == 0 && (call || tempreg == PIC_CALL_REG))
	    {
	      relax_start (offset_expr.X_add_symbol);
	      macro_build (&offset_expr, ADDRESS_LOAD_INSN, "t,o(b)", tempreg,
			   BFD_RELOC_MICROMIPS_CALL16, mips_gp_register);
	      relax_switch ();
	      macro_build (&offset_expr, ADDRESS_LOAD_INSN, "t,o(b)", tempreg,
			   BFD_RELOC_MICROMIPS_GOT_DISP, mips_gp_register);
	      relax_end ();
	...
	{
	  int gpdelay;
	  int lui_reloc_type = (int) BFD_RELOC_MICROMIPS_GOT_HI16;
	  int lw_reloc_type = (int) BFD_RELOC_MICROMIPS_GOT_LO16;
	  int local_reloc_type = (int) BFD_RELOC_MICROMIPS_GOT16;

	  /* This is the large GOT case.  If this is a reference to an
	     external symbol, and there is no constant, we want
	       lui	$tempreg,<sym>		(BFD_RELOC_MICROMIPS_GOT_HI16)
	       addu	$tempreg,$tempreg,$gp
	       lw	$tempreg,<sym>($tempreg) (BFD_RELOC_MICROMIPS_GOT_LO16)
	     or for lca or if tempreg is PIC_CALL_REG
	       lui	$tempreg,<sym>		(BFD_RELOC_MICROMIPS_CALL_HI16)
	       addu	$tempreg,$tempreg,$gp
	       lw	$tempreg,<sym>($tempreg) (BFD_RELOC_MICROMIPS_CALL_LO16)
	     For a local symbol, we want
	       lw	$tempreg,<sym>($gp)	(BFD_RELOC_MICROMIPS_GOT16)
	       nop
	       addiu	$tempreg,$tempreg,<sym>	(BFD_RELOC_MICROMIPS_LO16)
	     ...
	     For a local symbol, we want
	       lw	$tempreg,<sym>($gp)	(BFD_RELOC_MICROMIPS_GOT16)
	       nop
	       addiu	$tempreg,$tempreg,<constant> (BFD_RELOC_MICROMIPS_LO16)
	     ...
	     For a local symbol, we want
	       lw	$tempreg,<sym>($gp)	(BFD_RELOC_MICROMIPS_GOT16)
	       lui	$at,<hiconstant>
	       addiu	$at,$at,<loconstant>	(BFD_RELOC_MICROMIPS_LO16)
	       addu	$tempreg,$tempreg,$at
	  */
	  ...
	  if (expr1.X_add_number == 0 && breg == 0
	      && (call || tempreg == PIC_CALL_REG))
	    {
	      lui_reloc_type = (int) BFD_RELOC_MICROMIPS_CALL_HI16;
	      lw_reloc_type = (int) BFD_RELOC_MICROMIPS_CALL_LO16;
	    }
	  macro_build (&offset_expr, "lui", "t,u", tempreg, lui_reloc_type);
	  macro_build (NULL, ADDRESS_ADD_INSN, "d,v,t",
		       tempreg, tempreg, mips_gp_register);
      ...
      else if (mips_big_got && HAVE_NEWABI)
	{
	  ...
	  /* This is the large GOT case.  If this is a reference to an
	     ...
	     If we have NewABI, and we know it's a local symbol, we want
	       lw	$reg,<sym>($gp)		(BFD_RELOC_MICROMIPS_GOT_PAGE)
	       addiu	$reg,$reg,<sym>		(BFD_RELOC_MICROMIPS_GOT_OFST)
	     otherwise we have to resort to GOT_HI16/GOT_LO16.  */
	  ...
	  offset_expr.X_add_number = expr1.X_add_number;
	  macro_build (&offset_expr, ADDRESS_LOAD_INSN, "t,o(b)", tempreg,
		       BFD_RELOC_MICROMIPS_GOT_PAGE, mips_gp_register);
	  macro_build (&offset_expr, ADDRESS_ADDI_INSN, "t,r,j", tempreg,
		       tempreg, BFD_RELOC_MICROMIPS_GOT_OFST);
	  if (add_breg_early)
	    {
	      macro_build (NULL, ADDRESS_ADD_INSN, "d,v,t",
			   treg, tempreg, breg);
      ...
      if (offset_expr.X_op == O_constant)
	{
	  ...
	  if (breg != 0)
	    macro_build (NULL, ADDRESS_ADD_INSN, "d,v,t",
			 tempreg, tempreg, breg);
	  macro_build (&offset_expr, s, fmt, treg, BFD_RELOC_MICROMIPS_LO16,
		       tempreg);
	}
      else if (mips_pic == NO_PIC)
	{
	  /* If this is a reference to a GP relative symbol, and there
	     is no base register, we want
	       <op>	$treg,<sym>($gp)	(BFD_RELOC_MICROMIPS_GPREL16)
	     Otherwise, if there is no base register, we want
	       lui	$tempreg,<sym>		(BFD_RELOC_MICROMIPS_HI16_S)
	       <op>	$treg,<sym>($tempreg)	(BFD_RELOC_MICROMIPS_LO16)
	     If we have a constant, we need two instructions anyhow,
	     so we always use the latter form.
	     ...
	     GP relative symbol, we want
	       addu	$tempreg,$breg,$gp
	       <op>	$treg,<sym>($tempreg)	(BFD_RELOC_MICROMIPS_GPREL16)
	     Otherwise we want
	       lui	$tempreg,<sym>		(BFD_RELOC_MICROMIPS_HI16_S)
	       addu	$tempreg,$tempreg,$breg
	       <op>	$treg,<sym>($tempreg)	(BFD_RELOC_MICROMIPS_LO16)
	  ...
	  if (HAVE_64BIT_SYMBOLS)
		  ...
		  if (breg == 0)
		  ...
		  else
		    {
		      macro_build (NULL, ADDRESS_ADD_INSN, "d,v,t",
				   tempreg, breg, mips_gp_register);
		      macro_build (&offset_expr, s, fmt, treg,
				   BFD_RELOC_MICROMIPS_GPREL16, tempreg);
		    }
		  relax_switch ();
		}
	      ...
	      if (used_at == 0 && mips_opts.at)
		{
		  ...
		  macro_build (&offset_expr, "daddiu", "t,r,j", tempreg,
			       tempreg, BFD_RELOC_MICROMIPS_HIGHER);
		  if (breg != 0)
		    macro_build (NULL, "daddu", "d,v,t", AT, AT, breg);
		  macro_build (NULL, "dsll32", "d,w,<", tempreg, tempreg, 0);
		  macro_build (NULL, "daddu", "d,v,t", tempreg, tempreg, AT);
		  macro_build (&offset_expr, s, fmt, treg,
			       BFD_RELOC_MICROMIPS_LO16, tempreg);
		  used_at = 1;
		}
	      else
		{
		  ...
		  macro_build (NULL, "dsll", "d,w,<", tempreg, tempreg, 16);
		  macro_build (&offset_expr, "daddiu", "t,r,j", tempreg,
			       tempreg, BFD_RELOC_MICROMIPS_HI16_S);
		  macro_build (NULL, "dsll", "d,w,<", tempreg, tempreg, 16);
		  if (breg != 0)
		    macro_build (NULL, "daddu", "d,v,t",
				 tempreg, tempreg, breg);
	  ...
	  if (breg == 0)
		  ...
		  && !nopic_need_relax (offset_expr.X_add_symbol, 1))
		{
		  relax_start (offset_expr.X_add_symbol);
		  macro_build (&offset_expr, s, fmt, treg,
			       BFD_RELOC_MICROMIPS_GPREL16, mips_gp_register);
		  relax_switch ();
		}
	      macro_build_lui (&offset_expr, tempreg);
	      macro_build (&offset_expr, s, fmt, treg,
      ...
      else if (!mips_big_got)
	{
	  ...
	  /* If this is a reference to an external symbol, we want
	     ...
	     For NewABI, we want
	       lw	$tempreg,<sym>($gp)	(BFD_RELOC_MICROMIPS_GOT_PAGE)
	       <op>	$treg,<sym>($tempreg)   (BFD_RELOC_MICROMIPS_GOT_OFST)
	  ...
	  if (HAVE_NEWABI)
		...
		macro_build (NULL, ADDRESS_ADD_INSN, "d,v,t",
			     tempreg, tempreg, breg);
	      macro_build (&offset_expr, s, fmt, treg,
			   BFD_RELOC_MICROMIPS_GOT_OFST, tempreg);
	      break;
	    }
	  expr1.X_add_number = offset_expr.X_add_number;
	  offset_expr.X_add_number = 0;
	  ...
	  relax_switch ();
	  macro_build (&offset_expr, ADDRESS_ADDI_INSN, "t,r,j", tempreg,
		       tempreg, BFD_RELOC_MICROMIPS_LO16);
	  relax_end ();
	  if (breg != 0)
	    macro_build (NULL, ADDRESS_ADD_INSN, "d,v,t",
			 tempreg, tempreg, breg);
	  macro_build (&expr1, s, fmt, treg, BFD_RELOC_MICROMIPS_LO16, tempreg);
	}
      else if (mips_big_got && !HAVE_NEWABI)
	{
	  int gpdelay;
	  ...
	  macro_build (NULL, ADDRESS_ADD_INSN, "d,v,t", tempreg, tempreg,
		       mips_gp_register);
	  macro_build (&offset_expr, ADDRESS_LOAD_INSN, "t,o(b)", tempreg,
		       BFD_RELOC_MICROMIPS_GOT_LO16, tempreg);
	  relax_switch ();
	  if (gpdelay)
	    macro_build (NULL, "nop", "");
	  macro_build (&offset_expr, ADDRESS_LOAD_INSN, "t,o(b)", tempreg,
		       BFD_RELOC_MICROMIPS_GOT16, mips_gp_register);
	  load_delay_nop ();
	  macro_build (&offset_expr, ADDRESS_ADDI_INSN, "t,r,j", tempreg,
		       tempreg, BFD_RELOC_MICROMIPS_LO16);
	  relax_end ();
	  ...
	  || offset_expr.X_op == O_constant)
	{
	  /* If this is a reference to a GP relative symbol, we want
	       <op>	$treg,<sym>($gp)	(BFD_RELOC_MICROMIPS_GPREL16)
	       <op>	$treg+1,<sym>+4($gp)	(BFD_RELOC_MICROMIPS_GPREL16)
	     If we have a base register, we use this
	       addu	$at,$breg,$gp
	       <op>	$treg,<sym>($at)	(BFD_RELOC_MICROMIPS_GPREL16)
	       <op>	$treg+1,<sym>+4($at)	(BFD_RELOC_MICROMIPS_GPREL16)
	     If this is not a GP relative symbol, we want
	       lui	$at,<sym>		(BFD_RELOC_MICROMIPS_HI16_S)
	       <op>	$treg,<sym>($at)	(BFD_RELOC_MICROMIPS_LO16)
	       <op>	$treg+1,<sym>+4($at)	(BFD_RELOC_MICROMIPS_LO16)
	     If there is a base register, we add it to $at after the
	     lui instruction.  If there is a constant, we always use
	     the last case.  */
	  if (offset_expr.X_op == O_symbol
	...
	{
	  /* If this is a reference to an external symbol, we want
	       lw	$at,<sym>($gp)		(BFD_RELOC_MICROMIPS_GOT16)
	       nop
	       <op>	$treg,0($at)
	       <op>	$treg+1,4($at)
	     Otherwise we want
      ...
      else if (mips_big_got)
	{
	  ...
	  /* If this is a reference to an external symbol, we want
	       lui	$at,<sym>		(BFD_RELOC_MICROMIPS_GOT_HI16)
	       addu	$at,$at,$gp
	       lw	$at,<sym>($at)		(BFD_RELOC_MICROMIPS_GOT_LO16)
	       nop
	       <op>	$treg,0($at)
	       <op>	$treg+1,4($at)
	     Otherwise we want
	  ...
	  relax_start (offset_expr.X_add_symbol);
	  macro_build (&offset_expr, "lui", "t,u",
		       AT, BFD_RELOC_MICROMIPS_GOT_HI16);
	  macro_build (NULL, ADDRESS_ADD_INSN, "d,v,t",
		       AT, AT, mips_gp_register);
	  macro_build (&offset_expr, ADDRESS_LOAD_INSN, "t,o(b)",
		       AT, BFD_RELOC_MICROMIPS_GOT_LO16, AT);
	  load_delay_nop ();
	  if (breg != 0)
	    macro_build (NULL, ADDRESS_ADD_INSN, "d,v,t", AT, breg, AT);
	  /* Itbl support may require additional care here.  */
	...
	abort ();
      ...
      if (mask == M_PREF_OB || mask == M_CACHE_AB || mask == M_CACHE_OB)
	{
	  int cacheop = MICROMIPS_EXTRACT_OPERAND (CACHEOP, *ip);
	  macro_build (NULL, s, fmt, cacheop, (unsigned long) 0, AT);
	}
      else
	macro_build (NULL, s, fmt, treg, (unsigned long) 0, AT);
      break;
    ...
    sd_ob:
      gas_assert (HAVE_32BIT_ADDRESSES);
      macro_build (&offset_expr, s, "t,o(b)", treg, BFD_RELOC_MICROMIPS_LO16,
		   breg);
      offset_expr.X_add_number += 4;
      macro_build (&offset_expr, s, "t,o(b)", treg + 1,
		   BFD_RELOC_MICROMIPS_LO16, breg);
      break;
	...
	{
	  if (dreg == RA)
	    macro_build (NULL, "jalr", "mj", sreg);
	  else
	    macro_build (NULL, "jalr", "t,s", dreg, sreg);
	}
      else
	{
	  if (sreg != PIC_CALL_REG)
      ...
      else if (mips_pic == SVR4_PIC)
	{
	  /* If this is a reference to an external symbol, and we are
	     using a small GOT, we want
	       lw	$25,<sym>($gp)		(BFD_RELOC_MICROMIPS_CALL16)
	       nop
	       jalr	$ra,$25
	       nop
	       lw	$gp,cprestore($sp)
	     The cprestore value is set using the .cprestore
	     pseudo-op.  If we are using a big GOT, we want
	       lui	$25,<sym>		(BFD_RELOC_MICROMIPS_CALL_HI16)
	       addu	$25,$25,$gp
	       lw	$25,<sym>($25)		(BFD_RELOC_MICROMIPS_CALL_LO16)
	       nop
	       jalr	$ra,$25
	       nop
	       lw	$gp,cprestore($sp)
	     If the symbol is not external, we want
	       lw	$25,<sym>($gp)		(BFD_RELOC_MICROMIPS_GOT16)
	       nop
	       addiu	$25,$25,<sym>		(BFD_RELOC_MICROMIPS_LO16)
	       jalr	$ra,$25
	       nop
	       lw $gp,cprestore($sp)
	  ...
	  if (HAVE_NEWABI)
	      ...
	      if (! mips_big_got)
		{
		  relax_start (offset_expr.X_add_symbol);
		  macro_build (&offset_expr, ADDRESS_LOAD_INSN, "t,o(b)",
			       PIC_CALL_REG, BFD_RELOC_MICROMIPS_CALL16,
			       mips_gp_register);
		  relax_switch ();
		  macro_build (&offset_expr, ADDRESS_LOAD_INSN, "t,o(b)",
			       PIC_CALL_REG, BFD_RELOC_MICROMIPS_GOT_DISP,
			       mips_gp_register);
		  relax_end ();
		}
	      else
		{
		  ...
		  macro_build (NULL, ADDRESS_ADD_INSN, "d,v,t", PIC_CALL_REG,
			       PIC_CALL_REG, mips_gp_register);
		  macro_build (&offset_expr, ADDRESS_LOAD_INSN, "t,o(b)",
			       PIC_CALL_REG, BFD_RELOC_MICROMIPS_CALL_LO16,
			       PIC_CALL_REG);
		  relax_switch ();
		  macro_build (&offset_expr, ADDRESS_LOAD_INSN, "t,o(b)",
			       PIC_CALL_REG, BFD_RELOC_MICROMIPS_GOT_PAGE,
			       mips_gp_register);
		  macro_build (&offset_expr, ADDRESS_ADDI_INSN, "t,r,j",
			       PIC_CALL_REG, PIC_CALL_REG,
	      ...
	      else
		{
	      ...
	      macro_build (&offset_expr, ADDRESS_LOAD_INSN, "t,o(b)",
			   PIC_CALL_REG, BFD_RELOC_MICROMIPS_GOT16,
			   mips_gp_register);
	      load_delay_nop ();
	      macro_build (&offset_expr, ADDRESS_ADDI_INSN, "t,r,j",
			   PIC_CALL_REG, PIC_CALL_REG,
      ...
      else
	{
	  gas_assert (offset_expr.X_op == O_symbol
		  ...
		  && offset_expr.X_add_number == 0);
	  macro_build (&offset_expr, "lwc1", "T,o(b)", treg,
		       BFD_RELOC_MICROMIPS_LITERAL, mips_gp_register);
	  break;
	}
	...
	{
	  used_at = 1;
	  macro_build (&offset_expr, "ld", "t,o(b)", treg,
		       BFD_RELOC_MICROMIPS_LO16, AT);
	}
      else
	{
	  used_at = 1;
	  macro_build (&offset_expr, "lw", "t,o(b)", treg,
		       BFD_RELOC_MICROMIPS_LO16, AT);
	  if (treg != RA)
	    {
	      /* FIXME: How in the world do we deal with the possible
		 overflow here?  */
	      offset_expr.X_add_number += 4;
	      macro_build (&offset_expr, "lw", "t,o(b)",
			   treg + 1, BFD_RELOC_MICROMIPS_LO16, AT);
	    }
	}
      break;
      ...
      else
	{
	  ...
	  if (mips_opts.isa != ISA_MIPS1)
	    {
	      macro_build (&offset_expr, "ldc1", "T,o(b)",
			   treg, BFD_RELOC_MICROMIPS_LO16, AT);
	      break;
	    }
	  breg = AT;
	  r = BFD_RELOC_MICROMIPS_LO16;
	...
	load_register (AT, &imm_expr, dbl);
      macro_build (NULL, dbl ? "dmult" : "mult", "s,t", sreg, imm ? AT : treg);
      macro_build (NULL, "mflo", "mj", dreg);
      macro_build (NULL, dbl ? "dsra32" : "sra", "t,r,<", dreg, dreg, RA);
      macro_build (NULL, "mfhi", "mj", AT);
      if (mips_trap)
	macro_build (NULL, "tne", "s,t,q", dreg, AT, 6);
      else
	{
	  my_getExpression (&expr2, MICROMIPS_TARGET);
	  macro_build (&expr2, "beq", "s,t,p", dreg, AT);
	  macro_build (NULL, "nop", "", 0);
	  macro_build (NULL, "break", "mF", 6);
	  micromips_add_number_label ();
	}
	...
	{
	  my_getExpression (&expr2, MICROMIPS_TARGET);
	  macro_build (&expr2, "beq", "s,t,p", AT, 0);
	  macro_build (NULL, "nop", "", 0);
	  macro_build (NULL, "break", "mF", 6);
	  micromips_add_number_label ();
	}
	...
	rot = (64 - rot) & 0x3f;
	if (rot >= 32)
	  macro_build (NULL, "dror32", "t,r,<", dreg, sreg, rot - 32);
	else
	  macro_build (NULL, "dror", "t,r,<", dreg, sreg, rot);
	break;
      }
	...
	if (imm_expr.X_op != O_constant)
	  as_bad (_("Improper rotate count"));
	rot = imm_expr.X_add_number & 0x1f;
	macro_build (NULL, "rotr", "t,r,<", dreg, sreg, (32 - rot) & 0x1f);
	break;
      }
	...
	if (imm_expr.X_op != O_constant)
	  as_bad (_("Improper rotate count"));
	rot = imm_expr.X_add_number & 0x1f;
	macro_build (NULL, "rotr", "t,r,<", dreg, sreg, rot);
	break;
      }

    case M_SEQ:
      if (sreg == 0)
	macro_build (&expr1, "sltiu", "t,r,j", dreg, treg,
		     BFD_RELOC_MICROMIPS_LO16);
      else if (treg == 0)
	macro_build (&expr1, "sltiu", "t,r,j", dreg, sreg,
		     BFD_RELOC_MICROMIPS_LO16);
      else
	{
	  macro_build (NULL, "xor", "d,v,t", dreg, sreg, treg);
	  macro_build (&expr1, "sltiu", "t,r,j", dreg, dreg,
		       BFD_RELOC_MICROMIPS_LO16);
	}
      break;
      ...
      if (sreg == 0)
	{
	  ...
	  && imm_expr.X_add_number < 0x10000)
	{
	  macro_build (&imm_expr, "xori", "t,r,i", dreg, sreg,
		       BFD_RELOC_MICROMIPS_LO16);
	}
      else if (imm_expr.X_op == O_constant
	       && imm_expr.X_add_number > -0x8000
	       && imm_expr.X_add_number < 0)
	{
	  imm_expr.X_add_number = -imm_expr.X_add_number;
	  macro_build (&imm_expr, HAVE_32BIT_GPRS ? "addiu" : "daddiu",
		       "t,r,j", dreg, sreg, BFD_RELOC_MICROMIPS_LO16);
	}
      else
	{
	  load_register (AT, &imm_expr, HAVE_64BIT_GPRS);
    ...
    sge:
      macro_build (NULL, s, "d,v,t", dreg, sreg, treg);
      macro_build (&expr1, "xori", "t,r,i", dreg, dreg,
		   BFD_RELOC_MICROMIPS_LO16);
      break;
	...
	{
	  macro_build (&imm_expr, mask == M_SGE_I ? "slti" : "sltiu", "t,r,j",
		       dreg, sreg, BFD_RELOC_MICROMIPS_LO16);
	}
      else
	{
	  load_register (AT, &imm_expr, HAVE_64BIT_GPRS);
    ...
    case M_SLT_I:
	  ...
	  && imm_expr.X_add_number < 0x8000)
	{
	  macro_build (&imm_expr, "slti", "t,r,j", dreg, sreg,
		       BFD_RELOC_MICROMIPS_LO16);
	  break;
	}
      used_at = 1;
      ...
      if (sreg == 0)
	{
	  as_warn (_("Instruction %s: result is always true"),
		   ip->insn_mo->name);
	  macro_build (&expr1, HAVE_32BIT_GPRS ? "addiu" : "daddiu", "t,r,j",
		       dreg, 0, BFD_RELOC_MICROMIPS_LO16);
	  break;
	}
      if (imm_expr.X_op == O_constant
	  && imm_expr.X_add_number >= 0
	...
	as_bad (_("operand overflow"));
      if (! target_big_endian)
	++offset_expr.X_add_number;
      macro_build (&offset_expr, s, "t,o(b)", AT, BFD_RELOC_MICROMIPS_LO16,
		   breg);
      if (! target_big_endian)
	--offset_expr.X_add_number;
      else
	++offset_expr.X_add_number;
      macro_build (&offset_expr, "lbu", "t,o(b)", treg,
		   BFD_RELOC_MICROMIPS_LO16, breg);
      macro_build (NULL, "sll", "t,r,<", AT, AT, 8);
      macro_build (NULL, "or", "d,v,t", treg, treg, AT);
      break;

    case M_ULD:
      if (offset_expr.X_add_number >= 0x8000 - 7)
	as_bad (_("operand overflow"));
    case M_ULD_A:
      s = "ldl";
      s2 = "ldr";
      ...
      goto ulwa;
    case M_ULW:
      if (offset_expr.X_add_number >= 0x8000 - 3)
	as_bad (_("operand overflow"));
    case M_ULW_A:
      s = "lwl";
      s2 = "lwr";
	...
	expr1.X_add_number = 0;
      macro_build (NULL, s, fmt, treg, (unsigned long) expr1.X_add_number, AT);
      if (! target_big_endian)
	expr1.X_add_number = 0;
      else
	expr1.X_add_number = off;
      macro_build (NULL, s2, fmt, treg, (unsigned long) expr1.X_add_number, AT);
      break;
	...
	expr1.X_add_number = 0;
      macro_build (&expr1, "lbu", "t,o(b)", AT, BFD_RELOC_MICROMIPS_LO16, AT);
      macro_build (NULL, "sll", "t,r,<", treg, treg, 8);
      macro_build (NULL, "or", "d,v,t", treg, treg, AT);
      break;
	...
	as_bad (_("operand overflow"));
      if (target_big_endian)
	++offset_expr.X_add_number;
      macro_build (&offset_expr, "sb", "t,o(b)", treg,
		   BFD_RELOC_MICROMIPS_LO16, breg);
      macro_build (NULL, "srl", "t,r,<", AT, treg, 8);
      if (target_big_endian)
	--offset_expr.X_add_number;
      else
	++offset_expr.X_add_number;
      macro_build (&offset_expr, "sb", "t,o(b)", AT, BFD_RELOC_MICROMIPS_LO16,
		   breg);
      break;
	...
	macro_build (NULL, ADDRESS_ADD_INSN, "d,v,t", AT, AT, breg);
      if (! target_big_endian)
	expr1.X_add_number = 0;
      macro_build (&expr1, "sb", "t,o(b)", treg, BFD_RELOC_MICROMIPS_LO16, AT);
      macro_build (NULL, "srl", "t,r,<", treg, treg, 8);
      if (! target_big_endian)
	expr1.X_add_number = 1;
      else
	expr1.X_add_number = 0;
    ...
    case M_DSUB_I:
	  ...
	  && imm_expr.X_add_number <= 0x200)
	{
	  macro_build (NULL, "daddi", "t,r,.", dreg, sreg,
		       -imm_expr.X_add_number);
	  break;
	}
      used_at = 1;
      load_register (AT, &imm_expr, 1);
      macro_build (NULL, "dsub", "d,v,t", dreg, sreg, AT);
      break;
