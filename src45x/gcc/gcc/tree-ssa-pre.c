		 ...
		 basic_block pred, basic_block phiblock, bitmap seen)
{
  ...
  switch (expr->kind)
	...
	for (i = 0; i < newnary.length; i++)
	  {
		...
		leader = find_leader_in_sets (op_val_id, set1, set2);
                result = phi_translate_1 (leader, set1, set2, pred, phiblock, seen);
	...
	phi_trans_add (oldexpr, expr, pred); /* CS */
	return expr;
      }
      break;
	...
	for (i = 0, j = 0;
	     VEC_iterate (vn_reference_op_s, operands, i, operand); i++, j++)
	  {
		...
		leader = find_leader_in_sets (op_val_id, set1, set2);
		opresult = phi_translate_1 (leader, set1, set2, pred, phiblock, seen);
...
init_pre (bool do_fre)
{
  ...
  expression_to_id = htab_create (num_ssa_names * 3,
				  pre_expr_hash,
				  pre_expr_eq, NULL);
  seen_during_translate = BITMAP_ALLOC (&grand_bitmap_obstack); /* CS */
