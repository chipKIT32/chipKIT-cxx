...
static void grow_arg (const char *, int, int);
static const char *get_top_arg (void);
...
or with constant text in a single argument.
 ...
 %J     Join the current argument with the previous argument, if there is one
        (has no effect if there is no previous argument). Joining an argument
        when the previous argument was specified as a temp file name is
        undefined.
...
/* Return the string value of the top last argument added
 */
static const char *
get_top_arg (void)
{
  return argbuf_index ? argbuf [argbuf_index -1 ] : NULL;
}

/* Append the text to the argument at the end of the vector.
   This is done when a %J is seen.
...
static void
grow_arg (const char *arg, int delete_always, int delete_failure)
{
  char *string;
  ...
  /* not sure where we got the memory for the string, so we alloc
     new space to be sure we have room to append. */
  string = (char*)xmalloc (strlen (arg) + strlen (argbuf[argbuf_index - 1]) + 1 + 1);
  strcpy (string, argbuf[argbuf_index-1]);
  strcat (string, arg);
  argbuf[argbuf_index - 1] = string;
...
#define STRINGIZE(X) STRINGIZE2(X)
  ...
  /* transform executables using PROGRAM_PREFIX */
  info.name = xmalloc(sizeof("xc16-")+strlen(name)+1);
  if (info.name) 
    sprintf(info.name, "xc16-%s", name);
  else info.name = name;
#else
  info.name = name;
#endif
...
#elif !defined (CHIPKIT_PIC32) && defined(TARGET_MCHP_PIC32MX)
  fputs (_("\
  -relaxed-math            Use alternative floating-point support routines\n"), stdout);
  fputs (_("\
...
/* Nonzero means %J has been seen; the next arg to be terminated
   will be textually appended to the value (text) of the previous
   argument. */
static int this_is_joined_argument;
...
end_going_arg (void)
{
  if (arg_going)
      ...
      if (this_is_linker_script)
	{
      ...
      if (this_is_joined_argument)
        {
          grow_arg (string, delete_this_arg, this_is_output_file);
        }
      else
        {
	...
	outfiles[input_file_number] = get_top_arg();
      arg_going = 0;
    }
}
...
eval_spec_function (const char *func, const char *args)
{
  ...
  int save_this_is_joined_argument;
  ...
  save_this_is_joined_argument = this_is_joined_argument;
  ...
  this_is_joined_argument = save_this_is_joined_argument;
